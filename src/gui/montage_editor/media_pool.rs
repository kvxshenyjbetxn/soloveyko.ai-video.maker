...
pub(super) fn draw_media_pool(
    ...
    for file in &dropped_files {
        if let Some(path) = &file.path {
            ...
            if VALID_EXTS.contains(&ext.as_str())
                && !editor.media_pool.iter().any(|m| m.path == *path)
            {
    ...
    ui.horizontal(|ui| {
        ui.label(
            egui::RichText::new(format!(
                "📁 {}",
                translate(language, "montage_editor_media_pool")
            ))
            .strong(),
        );
        ui.with_layout(Layout::right_to_left(egui::Align::Center), |ui| {
            if ui
                .button(translate(language, "montage_editor_add_media"))
                .clicked()
            {
            ...
            let all_image_paths: Vec<PathBuf> = editor
                .media_pool
                .iter()
            ...
            if !all_image_paths.is_empty() {
                if ui
                    .small_button("🎬")
                    .on_hover_text(translate(language, "montage_editor_animate_all"))
                    .clicked()
                {
            ...
            let selected_image_paths: Vec<PathBuf> = editor
                .media_pool
                .iter()
                .filter(|m| {
                    matches!(m.kind, ClipKind::Image) && editor.selected_media_ids.contains(&m.id)
                })
                .map(|m| m.path.clone())
                .collect();
            if !selected_image_paths.is_empty() {
                ...
                if ui
                    .small_button(translate(language, "montage_editor_animate_selected"))
    ...
    ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            if editor.media_pool.is_empty() {
                ...
                let (rect, _) =
                    ui.allocate_exact_size(Vec2::new(ui.available_width(), drop_h), Sense::hover());
                ...
                ui.painter()
                    .rect(rect, 6.0, bg_col, Stroke::new(1.5, stroke_col));
                ui.painter().text(
            ...
            for (idx, media) in editor.media_pool.iter().enumerate() {
                ...
                let stock_seg_idx: Option<usize> = editor
                    .clips
                    .iter()
                ...
                ui.horizontal(|ui| {
                    let (rect, resp) =
                        ui.allocate_exact_size(Vec2::new(item_w, ITEM_H), Sense::click_and_drag());
                    ...
                    ui.painter()
                        .rect(rect, 4.0, bg, Stroke::new(1.0, stroke_col));
                    ...
                    ui.painter()
                        .rect_filled(thumb_area, 3.0, Color32::from_rgb(8, 8, 10));
                    ...
                    if matches!(media.kind, ClipKind::Audio) {
                        ...
                        if tex_size.x > 0.0 && tex_size.y > 0.0 {
                            ...
                            let img_rect =
                                Rect::from_center_size(thumb_area.center(), Vec2::new(dw, dh));
                            ui.painter().image(
                                texture.id(),
                                img_rect,
                    ...
                    if is_busy {
                        ui.painter()
                            .rect_filled(rect, 4.0, Color32::from_black_alpha(150));
                    ...
                    resp.context_menu(|ui| {
                        ...
                        if ui
                            .add_enabled(
                                !is_busy,
                                egui::Button::new(translate(language, "montage_editor_regen_same")),
                            )
                            .clicked()
                        {
            ...
            if let Some(pos) = ui.input(|i| i.pointer.hover_pos()) {
                ...
                let label = if media.name.chars().count() > 12 {
                    format!(
                        "{} {}…",
                        icon,
                        media.name.chars().take(10).collect::<String>()
                    )
                } else {
    ...
    if is_hovering_media && !editor.media_pool.is_empty() {
        ...
        painter.rect_filled(
            pool_rect,
            0.0,
            Color32::from_rgba_unmultiplied(9, 123, 244, 22),
        );
        painter.rect_stroke(
            pool_rect,
            6.0,
