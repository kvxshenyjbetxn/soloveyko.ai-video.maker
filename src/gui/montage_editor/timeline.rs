...
const MIN_TIMELINE_TRACK_HEIGHT: f32 = 36.0;
const MAX_TIMELINE_TRACK_HEIGHT: f32 = 120.0;

fn placeholder_title_font_size(track_h: f32) -> f32 {
    (track_h * 0.24).clamp(9.5, 16.0)
}

fn placeholder_body_font_size(track_h: f32) -> f32 {
    (track_h * 0.22).clamp(9.0, 16.0)
}
...
pub(super) fn draw_timeline(
    ...
    // Верхній drag-handle змінює тільки загальну висоту панелі таймлайна.
    // Правий повзунок нижче змінює тільки висоту елементів усередині таймлайна.
    let handle_h = 4.0;
    ...
    ui.horizontal(|ui| {
        ...
        let (labels_rect, _) =
            ui.allocate_exact_size(Vec2::new(label_w, timeline_view_h), Sense::hover());

        // Авто-скрол за плейхедом під час відтворення:
        // якщо плейхед виходить за межі видимої зони — підтягуємо таймлінію.
        let scroll_visible_w = (ui.available_width() - right_panel_w - right_panel_gap).max(120.0);
        ...
        let output = ui
            .allocate_ui_with_layout(
                Vec2::new(
                    scroll_visible_w + right_panel_gap + right_panel_w,
                    timeline_view_h,
                ),
                egui::Layout::right_to_left(egui::Align::Min),
                |ui| {
                    ui.allocate_ui_with_layout(
                        Vec2::new(right_panel_w, timeline_view_h),
                        ...
                        |ui| {
                            ...
                            if let Some(ref drag_id) = editor.dragged_media_id.clone() {
                                if let Some(pos) = mouse_pos {
                                    if rect.contains(pos) && pos.y >= rect.top() {
                                        ...
                                        if let Some(media) =
                                            editor.media_pool.iter().find(|m| &m.id == drag_id)
                                        {
                                            ...
                                            if clip_fits_track(&media.kind, target_kind) {
                                                ...
                                                let preview_w =
                                                    (media.duration_secs * zoom).max(4.0);
                                            ...
                                            {
                                                {
                                                    ...
                                                    let pair_uuid =
                                                        if matches!(kind, ClipKind::Video)
                                                            && has_audio
                                                        {
                                                            Some(uuid_str())
                                                        } else {
                                                    ...
                                                    .map(|(ti, _)| ti);
                                                            free.unwrap_or_else(|| {
                                                                ...
                                                                editor.num_tracks += 1;
                                                                editor.track_kinds.push(
                                                                    super::types::TrackKind::Audio,
                                                                );
                            ...
                            for clip in &clips_snapshot {
                                ...
                                if clip.is_placeholder {
                                    ...
                                    if cw <= 56.0 || clip_rect.height() < title_font + 10.0 {
                                        painter.text(
                                            ...
                                            &label,
                                            egui::FontId::proportional(title_font.max(10.0)),
                                        ...
                                        let line_height = (body_font * 1.2).max(10.0);
                                        let body_top = clip_rect.top() + title_font + 8.0;
                                        let body_avail_h =
                                            (clip_rect.bottom() - 4.0 - body_top).max(0.0);
                                        let max_lines =
                                            ((body_avail_h / line_height).floor() as usize).min(6);
                                        if max_lines > 0 {
                                            let char_w = (body_font * 0.58).max(5.2);
                                            let body_max_chars =
                                                (((clip_rect.width() - 12.0) / char_w).floor()
                                                    as usize)
                                                    .max(8);
                                            let body_text = wrap_placeholder_text(
                                                &clip.name,
                                                body_max_chars,
                                                max_lines,
                                            );
                                            if !body_text.is_empty() {
                                                painter.text(
                                                    Pos2::new(clip_rect.left() + 6.0, body_top),
                                                    Align2::LEFT_TOP,
                                                    body_text,
                                ...
                                let is_anim = clip
                                    ...
                                    .as_ref()
                                    .map(|p| {
                                        anim_loading.lock().unwrap().contains(p)
                                            || regen_paths.contains(p)
                                    })
                                    .unwrap_or(false);

                                let is_sel =
                                    editor.selected_clip_id.as_deref() == Some(clip.id.as_str());
                                ...
                                if is_anim {
                                    ...
                                    let center = clip_rect.center();
                                    let r =
                                        (cw * 0.5).min(clip_rect.height() * 0.38).min(9.0).max(4.0);
                                    let segs = 20usize;
                                    for s in 0..segs {
                                        let a0 = t * std::f32::consts::TAU
                                            + s as f32 * std::f32::consts::TAU / segs as f32;
                                ...
                                let pad_bot = 3.0;
                                let inner_h =
                                    clip_rect.height() - opacity_strip_h - pad_top - pad_bot;
                                let strip_y = clip_rect.top()
                                    + pad_top
                                    + (1.0 - clip.opacity.clamp(0.0, 1.0)) * inner_h;
                                ...
                                if cw > 18.0 {
                                    let title_font = (track_h * 0.24).clamp(10.0, 15.0);
                                    let meta_font = (track_h * 0.20).clamp(9.0, 13.0);
                                    let title_y =
                                        clip_rect.top() + (track_h * 0.12).clamp(5.0, 8.0);
                                    ...
                                    painter.text(
                                        Pos2::new(clip_rect.left() + handle_w + 2.0, title_y),
                                        Align2::LEFT_TOP,
                                        &label,
                                        egui::FontId::proportional(title_font),
                                        Color32::from_rgb(200, 200, 215),
                                    );
                                    let meta_y = title_y + title_font + 4.0;
                                    if cw > 50.0 && meta_y + meta_font <= clip_rect.bottom() - 4.0 {
                                        painter.text(
                                            Pos2::new(clip_rect.left() + handle_w + 2.0, meta_y),
                                            Align2::LEFT_TOP,
                                            format!("{:.1}s", clip.duration),
                                            egui::FontId::proportional(meta_font),
                                ...
                                if clip_resp.drag_started() {
                                    ...
                                    let start =
                                        ui.input(|i| i.pointer.press_origin()).or(mouse_pos);
                                ...
                                if let Some(ref path) = clip.path {
                                    ...
                                    let is_animating =
                                        anim_loading.lock().unwrap().contains(&clip_path)
                                    ...
                                    clip_resp.context_menu(|ui| {
                                        ...
                                        if matches!(clip_kind, ClipKind::Image) {
                                            if is_animating {
                                                ...
                                                .button(translate(
                                                    language,
                                                    "montage_editor_animate",
                                                ))
                                                .clicked()
                                            {
                                                editor
                                                    .pending_animate_paths
                                                    .push(clip_path.clone());
                                                ui.close_menu();
                                            }
                                            ui.separator();
                                        }
                                            ...
                                            .clicked()
                                        {
                                            ...
                                            editor.clips.retain(|c| c.id != clip_id_str);
                                            if editor.selected_clip_id.as_deref()
                                                == Some(clip_id_str.as_str())
                                            {
                            ...
                            for ti in 0..editor.num_tracks {
                                ...
                                for i in 1..track_clips.len() {
                                    ...
                                    if zone_to > b_start + 0.001 {
                                        ...
                                        let cell = 6.0_f32;
                                        let line_color =
                                            Color32::from_rgba_unmultiplied(255, 182, 72, 120);
                                        ...
                                        if btn_hover
                                            && ui.ctx().input(|inp| inp.pointer.primary_pressed())
                                        {
                                            editor.overlap_transition_popup =
                                                Some((b.id.clone(), btn_center));
                                        }
                                    }
                                }
                            }
                            ...
                            if editor.split_tool_active {
                                if let Some(pos) = mouse_pos {
                                    if rect.contains(pos) && pos.y >= rect.top() {
                                        ...
                                        painter.line_segment(
                                            [
                                                Pos2::new(split_x, top_y),
                                                Pos2::new(split_x, bottom_y),
                                            ],
        ...
        for vis in 0..total_rows {
            ...
            let grip_font = (track_h * 0.24).clamp(10.0, 16.0);
            let label_font = (track_h * 0.28).clamp(11.0, 18.0);
            painter.text(
                Pos2::new(lrect.left() + 4.0, name_rect.center().y),
                Align2::LEFT_CENTER,
                "⠿",
                egui::FontId::proportional(grip_font),
                Color32::from_rgba_unmultiplied(
                    text_color.r(),
                    text_color.g(),
                    text_color.b(),
            ...
            let slider_h = (track_h * 0.08).clamp(3.0, 6.0);
            let pad_x = 6.0;
            ...
            let handle_radius = (track_h * 0.12).clamp(5.0, 8.0);
            painter.circle_filled(handle_center, handle_radius, fill_color);
