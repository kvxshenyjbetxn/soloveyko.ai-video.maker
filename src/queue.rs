...
pub struct PipelineJob {
    ...
    /// Окрема історія чату для ізольованої генерації HyperFrames-кліпів.
    pub hyperframes_agent_chat: Arc<Mutex<Vec<AgentChatMessage>>>,
    /// Поточна сесія HyperFrames-агента для перегляду та продовження діалогу.
    pub hyperframes_agent_session: Arc<Mutex<Option<AgentSessionInfo>>>,
...
impl PipelineJob {
    pub fn new(id: u64, name: String, settings: JobSettings) -> Self {
        ...
        Self {
            ...
            hyperframes_agent_chat: Arc::new(Mutex::new(Vec::new())),
            hyperframes_agent_session: Arc::new(Mutex::new(None)),
