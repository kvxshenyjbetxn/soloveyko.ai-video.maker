...
pub fn call_claude_code_new_session_streaming(
    ...
    log(&format!(
        "Starting Claude CLI agent session. Model: {}, session: {}",
        model, session_id
    ));
    ...
    log(&format!(
        "Running: claude --model {} -p \"[prompt]\" --allowedTools Bash,Write,Read --dangerously-skip-permissions --output-format stream-json --session-id {}",
        model, session_id
    ));
    ...
    loop {
        ...
        while let Some(pos) = line_buf.find('\n') {
            ...
            if let Some(text) =
                format_claude_json_event(trimmed, &mut final_result, &mut acc_in, &mut acc_out)
            {
                on_chunk(&text);
            }
        }
    ...
    if !remaining.is_empty() {
        if let Some(text) =
            format_claude_json_event(&remaining, &mut final_result, &mut acc_in, &mut acc_out)
        {
            on_chunk(&text);
        }
    }
...
pub fn call_claude_code_resume(
    ...
    if output.status.success() {
        ...
        let response =
            parse_claude_json_response(&stdout).unwrap_or_else(|| stdout.trim().to_string());
...
pub fn call_claude_code(
    ...
    log(&format!(
        "Starting Claude CLI translation. Model: {}",
        model
    ));
    ...
    log(&format!(
        "Running: claude --model {} -p \"[prompt and script text]\"",
        model
    ));
