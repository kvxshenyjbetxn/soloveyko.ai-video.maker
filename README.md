...
**Відновлення null-медіа при завантаженні (`load_timeline_clips`):** якщо сегмент у `timeline.json` має `"media": null` і водночас є поле `"text"` (ознака pipeline-формату від агента, не редакторські gap-сегменти), `load_timeline_clips` автоматично відновлює медіашлях через позиційне відображення — той самий алгоритм що й `assign_media_to_timeline`. Сортує файли `media/` за іменем і призначає `media/files[floor(i * n_files / n_segs)]`. Захищає від випадкового очищення агентом поля `media` в таймлайні.
