...
- **Стабільна розмітка вікна:** Розрахунок колонок (`cols`) для `egui::Grid` здійснюється на основі `screen.width()` (базового розміру вікна програми) замість динамічного `available_width()`. Це повністю вирішило проблему з feedback-петлею нескінченного автоматичного розширення вікна пікера за межі екрана. Додатково задано максимальне обмеження розміру та додано підтримку `resizable(true)`.
...
- **Самовільне розширення вікна (Feedback loop) в egui**: Розрахунок кількості колонок (`cols`) для `egui::Grid` у Stock Picker, який базувався на `ui.available_width()` всередині `ScrollArea` з увімкненим горизонтальним перенесенням, призводив до нескінченної петлі зворотного зв'язку: збільшення вікна → збільшення доступної ширини → перерахунок колонок → розширення Grid → додаткове розширення ScrollArea та всього вікна. Вирішено шляхом використання стабільного parent-контексту `screen.width()` (базової ширини всього екрана) для лімітування максимальної кількості колонок, що дозволило зберегти вікно масштабованим (`resizable(true)`) без самовільного роздування.
