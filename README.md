...
│   │   ├── resume.rs            — FoundFiles (text.txt / voice / subtitle.srt / **timeline.json** / media/ / output_video), ResumePendingData (keep_voiceover / keep_subtitles / **keep_timeline** / keep_video), draw_resume_dialog: перевірка наявних файлів задачі та діалог відновлення з чекбоксами; наявність `timeline.json` → окремий пункт «Таймлайн» з чекбоксом — якщо знято, агент перезапускається; якщо залишено — `skip_agent_on_resume = true` і пошук медіа стартує без агента
