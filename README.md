...
│   │   ├── video.rs             — секція відеоряду (сервіс Googler, вибір LLM для генерації промтів, режим нарізання тексту, пріоритети зображень, промт); при виборі Claude Code, Gemini CLI, Codex CLI або AGY CLI поле video_prompt ховається і показується video_agent_prompt (додатковий промт користувача — склеюється з системною інструкцією з agent_prompts.rs); toggle «Вказати стиль» розкриває поле video_style_prompt — якщо увімкнено, {{text}} замінюється текстом з поля text у timeline.json і цей складений промт передається у Googler замість голого тексту; якщо вимкнено — text йде напряму (поведінка до введення цього поля); панель пріоритетів відеопровайдерів має чекбокс вмикання/вимикання поряд з кожним провайдером (`googler_video_disabled: &mut Vec<String>`) — вимкнені провайдери фільтруються до передачі в `build_job_settings` і відображаються слабшим (weak) кольором; режим нарізки тексту зберігається окремо для не-CLI сервісів у `text_split_mode_openrouter` — при переключенні на CLI зберігається поточний вибір, при поверненні — відновлюється (щоб OpenRouter та агенти не перезаписували налаштування один одного)
...
- **Топбар** (зверху) — назва задачі, повзунок масштабу таймлінії (`timeline_zoom`), дві кнопки вибору рендеру: **«🎬 FFmpeg»** (зелена) та **«✂️ CapCut»** (синя), а також кнопка розгортання на весь екран / згортання у вікно (**`🗖`** / **`🗗`**).
...
- **Зображення** — декодуються через крейт `image` (`std::fs::read` → `image::load_from_memory` → `thumbnail(640, 1280)` → `.save` як JPG `000001.jpg`). Жодного FFmpeg. До 4 спроб з затримками [0, 400ms, 1.2s, 3.5s] — якщо файл ще пишеться пайплайном у момент відкриття редактора. Після останньої спроби `extraction_complete` ставиться в `true` незалежно від результату (щоб ⏳ не зависав вічно).
...
**`draw_montage_editor_window` повертає `MontageEditorActions`** — структуру з двома полями: `animate_paths: Vec<PathBuf>` (зображення для оживлення image-to-video) та `regen_action: Option<(file, settings, is_custom, job_id, job_name)>` (запит перегенерації медіа). `app.rs` обробляє ці дії поза редактором: `animate_paths` передаються у `animate_single_image` (той самий `gallery_anim_loading` що й у Галереї), `regen_action` або відкриває `media_regen_window` (is_custom=true) або одразу викликає `regenerate_single_media`. Такий підхід уникає дублювання логіки між Галереєю і редактором монтажу. Функція приймає `regen_paths: &HashSet<PathBuf>` (знімок `media_regen_paths` за кадр) — передається у `draw_media_pool` та `draw_timeline` для відображення спінерів під час перегенерації у будь-якому з цих місць. **`preview_stale_path` у fullscreen-прев'ю:** якщо `editor.preview_stale_path == Some(preview_path)` — кадр N: скидаємо стару текстуру (`pool_preview_texture = None`) та показуємо спінер-заглушку; кадр N+1: стара текстура вже звільнена GPU, завантажуємо нову. Без цього затримки при зміні розмірів зображення давали артефакт «половина по горизонталі сіра».
...
- **Виклик Claude CLI (`claude`), Gemini CLI (`gemini`) та Codex CLI (`codex`)**:
  ...
  - **Автоматичне приховування налаштувань нарізки тексту:** Для Claude Code, Gemini CLI та Codex CLI у секції відеоряду автоматично приховуються налаштування режиму нарізки, оскільки ці локальні інструменти за замовчуванням мають працювати виключно з повним текстом сценарію без нарізання на дрібні сегменти. Режим нарізки для OpenRouter/None та CLI-сервісів **незалежний**: `text_split_mode` — активне значення (передається у пайплайн), `text_split_mode_openrouter` — збережений вибір для не-CLI сервісів. При переключенні OpenRouter→CLI зберігається в `text_split_mode_openrouter`; при CLI→OpenRouter відновлюється назад. Це запобігає ситуації коли повернення з агента скидало вибраний раніше режим до "full".
