...
│   │   ├── frame_cache.rs       — FrameCache: неблокуючий LRU з dual-quality кешем (scrub/sharp), обмеженим async префетчингом (до 2 кадрів) та gen-on-demand для still-кадрів
...
- **`FrameCache::get_frame(ctx, media, time, sharp_when_idle, settings)`** — LRU кеш (за замовчуванням 80 текстур). Ключ: `{media_id}_s_{frame_idx:06}` (scrub) або `{media_id}_h_{frame_idx:06}` (sharp). **Повністю неблокуючий**: якщо кадру нема в кеші, UI не декодує JPEG синхронно — ставить його в фонову чергу (до 3 паралельних завантажень) і одразу повертає найближчий попередній scrub-кадр. При LRU hit тихо префетчить 2 наступні кадри. При `sharp_when_idle=true` (плейхед не рухається) генерує чіткий still-кадр через `generate_sharp_frame` — однокадровий ffmpeg з seek. Для зображень sharp-кадр створюється через `image` crate. Ключі з префіксом `s_` (scrub) і `h_` (sharp) живуть в одному LRU, що гарантує коректний fallback без стрибків між якостями.
