...
│   ├── queue.rs                 — draw_queue_panel: нижня панель черги задач (картки, прогрес, лог-вікно); draw_queue_jobs_list: виокремлений список карток для повторного використання у fullscreen-режимі; кнопка 📂 відкриває папку задачі у файловому менеджері ОС
...
- **Вкладка Галерея (`Tab::Gallery`):** відображає адаптивну сітку прев'ю медіафайлів (`jpg/jpeg/png/webp` + відео `mp4/webm/mov`) з папок `{task_dir}/media/` для всіх задач черги. Сітка будується через `ui.horizontal_wrapped()` — мініатюри автоматично переносяться на новий рядок залежно від ширини вікна. Кожна задача — розкривний `CollapsingState` з кількістю файлів у заголовку. Вкладка видима в навбарі лише якщо хоча б одна задача має непорожню `media/`. Якщо жодної задачі немає — `active_tab` скидається на `Main`. На вкладці Галерея черга (`queue_panel`) прихована щоб не займати місце. Якщо є задача у стані `AwaitingMediaControl` — у верхньому правому куті вкладки з'являється кнопка «Продовжити обробку», яка сигналізує condvar і відновлює пайплайн. Авто-перехід на Gallery: у `update()` щоразу перевіряється чи є задача у стані `AwaitingMediaControl` — якщо так, `active_tab = Tab::Gallery`. Клік по картці задачі `AwaitingMediaControl` у черзі теж перемикає на Gallery. Текстури зображень кешуються у `gallery_textures: HashMap<PathBuf, Option<TextureHandle>>` — `None` означає "ще завантажується або помилка". Завантаження **асинхронне**: при першому рендері картки шлях потрапляє у `image_load_requests`, після відмалювання фрейму запускається `start_image_loading` у фоновому потоці (читання файлу + декодування `image = "0.25"` → `ColorImage` → `ctx.load_texture`), результат накопичується у `gallery_image_result: Arc<Mutex<Vec<(PathBuf, Option<TextureHandle>)>>>` і дренується в `gallery_textures` на наступному тику. **Важливо:** до переходу на асинхронне завантаження `load_image_texture` викликався синхронно у циклі рендерингу — 50+ зображень блокували UI на кілька секунд. Відеофайли (`mp4/webm/mov`) відображаються з першим кадром як мініатюра — вилучається через `ffmpeg pipe` без тимчасових файлів; поки thumbnail завантажується — показується темний прямокутник 16:9 з ▶.
