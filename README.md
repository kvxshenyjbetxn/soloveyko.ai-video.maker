...
│   │   ├── state.rs             — MontageEditorState: весь стан редактора; load() — завантаження timeline.json + пул + UUID-sync; save_to_timeline() — запис timeline.json з segments/overlay_tracks; `preview_stale_path: Option<PathBuf>` — шлях для відкладеного (на 1 кадр) перезавантаження preview-текстури після перегенерації
...
│   │   └── timeline.rs          — draw_timeline: ruler, N доріжок, sticky labels, drag Move/TrimLeft/TrimRight, магнітний снеп, ghost preview, кнопки «+ Відео»/«+ Аудіо», жовта сітка зон накладання (crosshatch); приймає `regen_paths: &HashSet<PathBuf>` — кліп крутить спінер якщо файл у наборі або анімується
...
- **Таймлінія** (внизу) — горизонтальний ScrollArea з лінійкою; N доріжок (відео V1, V2, … та аудіо A1, A2, …) + опційна голосова доріжка `♪ Voice`; кольорові блоки (синій=відео, фіолетовий=зображення, зелений=аудіо); лінія відтворення; повна підтримка drag. У шапці таймлінії (над доріжками) розміщені дві кнопки: **«+ Відео»** (додає відеодоріжку зверху, зсуваючи всі кліпи вниз) та **«+ Аудіо»** (додає аудіодоріжку знизу). Кожна доріжка має тип (`TrackKind::Video` / `TrackKind::Audio`) і відповідний лейбл (V1/V2… або A1/A2…). Голосова доріжка `♪ Voice` інтерлівінгується серед звичайних — її позиція (`voiceover_track_idx`) змінюється вертикальним перетягуванням. **Зони накладання:** при перетягуванні кліпа на інший на тій самій доріжці з'являється жовта діагональна сітка (crosshatch, `rgba(255,182,72,…)`), що позначає область майбутнього переходу.
...
- **Переходи (11 груп xfade + overlap-кросфейд):** два типи переходів. **Налаштовані:** визначаються через `clip_offset = t - clip_start_secs` відносно `transition_duration` (JobSettings). Fade — cross-dissolve через alpha. WipeLeft/Right/Up/Down — `painter_at(sub_rect)` відсікає рендер по осі переходу. SlideLeft/Right/Up/Down — `rect.translate()` зміщує контейнер за межі. FadeBlack/FadeWhite — чорний/білий rect поверх через alpha. **Автоматичний overlap-кросфейд:** коли два кліпи на одній доріжці накладаються (перекриваються в часі), в області накладання автоматично показується crossfade незалежно від налаштувань — outgoing кліп плавно згасає (alpha 1→0), incoming проявляється (alpha 0→1). Прогрес обчислюється як `(ph - overlap_start) / overlap_duration`. Для кожного кліпу враховуються його власні per-clip налаштування зуму/шейку.
...
- **Загальні поля:** `total_duration_secs` (загальна тривалість відео), `audio_start_secs` (зсув оригінальної озвучки у секундах, за замовчуванням `0.0`), `voiceover_track_idx` (позиція голосової доріжки: `0` = перед V1, `num_tracks` = знизу).
