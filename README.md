...
**4 зони інтерфейсу:**
- **Медіа пул** (ліворуч) — сканує `{save_path}/media/` та кореневі аудіофайли (`voice.mp3`/`voice.wav`). Кнопка «+ Додати» відкриває `rfd::FileDialog` для додавання файлів ззовні. Перетягування файлу з пулу на таймлінію додає кліп з плаваючою підказкою (ghost card). При додаванні зображення — дефолтна тривалість 5 секунд; відео та аудіо — `ffprobe -show_entries format=duration` (функція `probe_duration`).
- **Прев'ю + транспортні кнопки** (центр) — відображає кадр активного кліпу (трек 0) на поточній позиції плейхеду з ефектами зуму, покачування та переходами між кліпами. Транспортні кнопки ⏮ ⏹ ▶/⏸ ⏭ розташовані **під відео**, над таймлінією. При відтворенні паралельно програється аудіо-трек (`voice.wav`/`voice.mp3`). Прев'ю центрується вручну через `pad_x = (avail_w - frame_w) / 2.0` (без `vertical_centered` — він спричиняв дублювання кнопок).
