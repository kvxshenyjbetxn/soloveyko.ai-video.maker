...
- **`JobStatus`** — `Pending → Running / AwaitingControl (пауза для контролю перекладу) / AwaitingMediaControl (пауза для перегляду зображень) / AwaitingMontageControl (пауза перед монтажем для редактора) → Done / Failed(String)`. Обгорнутий у `Arc<Mutex<T>>`, бо змінюється з фонового потоку. `AwaitingMontageControl` відображається зеленим.
...
- **`agent_session: Arc<Mutex<Option<AgentSessionInfo>>>`** — активна сесія агента (`session_id`, `service`, `model`). Заповнюється завжди після успішного завершення `run_agent_timeline` (незалежно від будь-яких перемикачів). `None` — сесія ще не стартувала.
...
- **Кнопка «Перебудувати таймлінію»** — показується коли є збережена сесія (`agent_session` не `None`). Встановлює прапор `timeline_rebuild_requested = true`. `app.rs` перехоплює прапор кожен UI-кадр, порівнює поточні тексти сегментів `timeline.json` з `media/segment_texts.json`, запускає фонову перегенерацію змінених медіафайлів зі спінером у галереї та редакторі, і перезавантажує `MontageEditorState`.
...
**Взаємодія з агентом після завершення пайплайну:**

Після завершення `run_agent_timeline` пайплайн продовжується автоматично (без зупинки). Якщо пізніше потрібно скоригувати `timeline.json` через чат:

1. Відкрити вікно чату 💬 → написати агенту → агент редагує `timeline.json` на диску.
2. Натиснути «Перебудувати таймлінію» — `timeline_rebuild_requested = true`.
3. `app.rs` у наступному кадрі виявляє прапор: порівнює тексти сегментів нового `timeline.json` з збереженими в `media/segment_texts.json`, запускає `regenerate_single_media` лише для змінених, паралельно перезавантажує редактор монтажу.
4. Змінені медіафайли показуються зі спінером у галереї та редакторі, як при звичайній перегенерації.
