...
- **Прев'ю + транспортні кнопки** (центр) — відображає кадр виділеного кліпу на поточній позиції. Транспортні кнопки ⏮ ⏹ ▶/⏸ ⏭ розташовані **під відео**, над таймлінією. Прев'ю центрується вручну через `pad_x = (avail_w - frame_w) / 2.0` (без `vertical_centered` — він спричиняв дублювання кнопок).
...
**Система кадрового прев'ю (LRU cache):**
- `frame_cache: HashMap<String, TextureHandle>` — кеш до 100 записів (LRU: при переповненні видаляється найстаріший).
- Ключ кешу: для зображень — `"img-{clip_id}"`, для відео — `"vid-{clip_id}-{(offset*10) as u32}"`.
- Завантаження у фоновому потоці: `pending_frame: Arc<Mutex<Option<(String, Option<TextureHandle>)>>>`. `Some((key, Some(tex)))` — успіх, `Some((key, None))` — помилка. `loading_key: Option<String>` — ключ що зараз завантажується (блокує повторний запит).
- **Зображення** — декодуються через крейт `image` (jpg/png/webp), стискаються до `thumbnail(800, 800)` → `to_rgba8()` → `ColorImage` → `load_texture`. Жодного FFmpeg.
- **Відео** — `ffmpeg -vcodec ppm -f image2pipe` (PPM формат самоописний: `P6\n{w} {h}\n255\n{RGB data}`). Функція `parse_ppm_header()` читає заголовок і повертає `(w, h, pixel_offset)` — розмір не треба знати заздалегідь. RGB конвертується у RGBA побайтово. Надійніше за `-f rawvideo`, бо `read_exact` не блокує при розбіжності розмірів.
