...
| [Редактор монтажу](#редактор-монтажу-srcguimontage_editor) | ~223 | 5 зон: медіа-пул, прев'ю+транспорт, інспектор, таймлінія, топбар з кнопками FFmpeg/CapCut |
| [Stock Picker (`stock_picker.rs`)](#stock-picker-srcguistock_pickerrs) | ~349 | Pexels/Pixabay-пікер: вибір кількох сервісів одночасно, паралельний пошук, lazy search, фонове декодування без фрізів UI, trim editor з ffmpeg |
...
| [Черга задач (`queue.rs`)](#черга-задач-srcqueuers) | ~361 | `JobSettings`, `JobStatus`, `PipelineJob`, arc-поля стану, `capcut_mode_override` |
...
│   ├── googler.rs               — check_key + fetch_balance (ліміти зображень/відео) + GooglerImageLimiter + GooglerVideoLimiter (локальний підрахунок активних потоків генерації); підтримує як v4 API (image/video через `/api/v4/{provider}/...`), так і v5 API (`/api/v5/generations` → poll `/api/v5/generations/{id}` → завантаження з `storage.fast-gen.ai`); провайдери v5: `flow_fast` (Veo 3.1 Fast), `flow_light` (Veo 3.1 Light), `flow_quality` (Veo 3.1 Quality), `flow_omni_flash` (Omni Flash); при HTTP 429 з кодом `rate_limit.concurrency_exceeded` — нескінченний retry без лічильника спроб і без перемикання провайдера; при `rate_limit.hourly_exceeded` (годинний ліміт вичерпано) — нескінченний retry з паузою **5 хвилин** між спробами, також без зміни провайдера — чекає поки ліміт відновиться
│   ├── claude.rs                — call_claude_code_new_session_streaming (--output-format stream-json --verbose: читає NDJSON рядки в реальному часі, парсить події через format_claude_json_event, on_chunk отримує читабельний текст) + call_claude_code_resume (--resume + --output-format stream-json, результат витягується через parse_claude_json_response) + call_claude_code (звичайний виклик, allow_tools: bool) + ClaudeLimiter (семафор); запуск через `new_direct_cli_command("claude")` без cmd /C
│   ├── gemini.rs                — call_gemini_new_session_streaming (--session-id, spawn()+BufReader, on_chunk викликається один раз з parsed JSON у кінці) + call_gemini_resume (--resume) + call_gemini_cli (звичайний виклик, --yolo) + GeminiLimiter (семафор); запуск через `new_direct_cli_command("gemini")` без cmd /C
...
│       ├── agent_prompts.rs     — VIDEO_AGENT_SYSTEM_PROMPT: зашита системна інструкція для CLI-агента відеоряду (формат timeline.json, правила запису); плейсхолдери {{srt}}/{{path}} замінюються перед відправкою; окреме правило для СТВОРЕННЯ vs РЕДАГУВАННЯ: при створенні `media: null`, при редагуванні — зберігати наявні шляхи без змін (null тільки для нових сегментів); редагуй тільки цей файл щоб змінити поведінку агента
│       ├── capcut/
...
│   │   ├── types.rs             — константи (FRAME_CACHE_SIZE=80) + типи: PreviewQuality (Performance/Balanced/High/Ultra), PreviewRenderSettings (якість+FPS превʼю), ClipKind, TrackKind (Video/Audio), DragMode, EditorClip, MontagePreviewSettings, MontageEditorActions (animate_paths + regen_action + open_stock_picker + preview_render_changed), ClipDragState, PreviewDragMode, PreviewDragState, OpacityDragState; `EditorClip.opacity: f32` — прозорість кліпу 0.0–1.0; `EditorClip.stock_seg_idx: Option<usize>` — індекс у `stock_cache.json`
│   │   ├── utils.rs             — path_hash, frame_cache_dir + sharp_frame_cache_dir (з версіонуванням якості/FPS), probe_duration, uuid_str
...
│   │   ├── frame_cache.rs       — FrameCache: неблокуючий LRU з dual-quality кешем (scrub/sharp), обмеженим async префетчингом (до 2 кадрів) та gen-on-demand для still-кадрів; `cached_near_frame` двонаправлений — шукає найближчий кадр в обидва боки для плавного скрабінгу назад
│   │   ├── state.rs             — MontageEditorState: весь стан редактора; load() — завантаження timeline.json + пул + UUID-sync; save_to_timeline() — запис timeline.json з segments/overlay_tracks (зберігає оригінальні start_secs для overlap-зон, а також `opacity`); `overlap_transition_popup` — стан випадаючого списку вибору xfade-переходу; `opacity_drag: Option<OpacityDragState>` — стан перетягування смужки прозорості
...
│   │   ├── inspector.rs         — draw_inspector: час/тривалість/доріжка DragValue, scale/pos_x/pos_y Slider, слайдер прозорості (opacity 0.0–1.0), ефекти zoom/shake, видалення кліпу
...
│   │   ├── preview.rs           — draw_preview: adaptive quality (scrub/sharp), UV-based зум/покачування, 11 груп xfade + overlap-кросфейд (автоматичний crossfade при накладанні кліпів), overlay compositing з підтримкою opacity (alpha-blending через `render_clip_frame` alpha-параметр), transport controls, PreviewDragState для drag-трансформу overlay-кліпів
│   │   └── timeline.rs          — draw_timeline: ruler, N доріжок, sticky labels, drag Move/TrimLeft/TrimRight, магнітний снеп, ghost preview, кнопки «+ Відео»/«+ Аудіо», жовта сітка зон накладання (crosshatch) + кнопка ◈ вибору xfade-переходу (попап з 49 варіантами); смужка прозорості — біла горизонтальна лінія в межах кліпу, перетягуванням якої змінюється opacity (вгору = 1.0, вниз = 0.0); детекція зони через `press_origin()` (не `hover_pos()`) щоб уникнути зміщення при drag threshold; `opacity_drag` і `clip_drag_state` взаємно виключають одне одного; приймає `regen_paths: &HashSet<PathBuf>`
...
│   │   ├── preview.rs           — load_image_texture, draw_image_preview: повноекранний перегляд зображення; start_image_loading: асинхронне завантаження зображення у фоновому потоці (аналогічно start_thumbnail_extraction для відео)
...
│   │   ├── resume.rs            — FoundFiles (text.txt / voice / subtitle.srt / **timeline.json** / media/ / output_video / **expected_media**), ResumePendingData (keep_voiceover / keep_subtitles / **keep_timeline** / keep_video), draw_resume_dialog: перевірка наявних файлів задачі та діалог відновлення з чекбоксами; наявність `timeline.json` → окремий пункт «Таймлайн» з чекбоксом — якщо знято, агент перезапускається; якщо залишено — `skip_agent_on_resume = true` і пошук медіа стартує без агента; `expected_media` рахується з кількості сегментів у `timeline.json` — якщо медіафайлів менше ніж сегментів, з'являється кнопка «Догенерувати відсутні» → `enqueue_fill_missing` (стартує з Video-етапу, `skip_existing_media = true`, пропускає файли що вже є)
...
│   │   ├── translation_control.rs — TranslationControlWindowState (стан одного вікна контролю: text_input, regen_* поля, regen_result Arc, regen_loading Arc) + draw_translation_control_windows: рендерить усі відкриті вікна контролю з циклу по HashMap<u64, TranslationControlWindowState>; кожне вікно має унікальний egui::Id — кілька вікон можуть бути відкриті одночасно для різних задач
│   │   ├── voiceover.rs         — секція озвучки (провайдер "Voice Bot" / "Edge TTS", вибір голосу, темп/тональність/гучність)
│   │   ├── video.rs             — секція відеоряду: два візуальних режими — «Режим роботи» (API: OpenRouter/Без ЛЛМ | Агент: Claude/Gemini/Codex/AGY) та «Режим відеоряду» (Генерація: Googler | Стоки: Pexels/Pixabay); в API-режимі показується video_prompt з плейсхолдером {{text}}, в агентному — video_agent_prompt (склеюється з системною інструкцією agent_prompts.rs); toggle «Вказати стиль» → video_style_prompt; панель пріоритетів відеопровайдерів з чекбоксами вмикання/вимикання (`googler_video_disabled`); `text_split_mode_openrouter` зберігає режим нарізки при переході між API/Агент
...
- **Два окремих вікна для балансів і потоків:**
  ...
  - `draw_threads_window` — тільки потоки зі слайдерами: OR (1–25), Claude (1–25), Gemini (1–25), AGY (1–25), VoiceBot (тільки відображення, фіксовано 5), EdgeTTS (1–25), Googler img/vid (5–25), FFmpeg (1–8). Колір активних потоків через `thread_load_color`.
...
- **Нижня панель черги та ресайз:** `TopBottomPanel::bottom("queue_panel")` рендериться після правої бічної панелі пайплайну, тому вона візуально обмежується нею та не заходить під неї. Підтримує три стани: **звичайний** (resizable, min 140px / default 160px / max 350px), **згорнутий** (`collapsed = true` → `exact_height(32px)`, тільки шапка), **повноекранний** (`fullscreen = true` → `exact_height(32px)` в нижній панелі + список карток переїздить у `CentralPanel`). **Кнопка згортання (▼/▲):** заповнений трикутник намальований вручну через `ui.painter().add(Shape::convex_polygon(...))` — не Unicode-символ, щоб уникнути квадратів через відсутній гліф у шрифті. Колір змінюється при hover. **Кнопка повноекранного режиму (4 куточки):** намальована через `painter.line_segment(...)` — при `fullscreen=false` куточки спрямовані назовні, при `fullscreen=true` — всередину (стандартна exit-fullscreen іконка). Видна тільки якщо панель не згорнута. **`draw_queue_jobs_list`** — виокремлена публічна функція, що малює ScrollArea з картками. Викликається з `draw_queue_panel` (коли `!collapsed && !fullscreen`) і з `CentralPanel` у fullscreen-режимі. При виклику з `CentralPanel` обгорнута у `Frame::none().inner_margin(Margin { left: 8, top: 8, .. })`. При виклику з нижньої панелі — `inner_margin { top: 10, bottom: 8, .. }` без лівого відступу. **Загальна вартість черги:** зелений лейбл `$X.XXXXX` відображається **зліва від загального прогресбару** — сума `total_cost` усіх задач у черзі (переклади + відеопромти + перегенерації). Показується лише якщо сума > 0. При розрахунку ширини прогресбару ширина лейблу резервується через `painter.layout_no_wrap` — бар не перекриває текст. **Загальна смужка прогресу:** по центру верхнього рядка (шапки) черги задач інтегровано красивий анімований прогресбар `egui::ProgressBar` висотою `10.0` пікселів, який відображає загальний хід виконання всієї черги (середній прогрес усіх задач), з'являється одразу при появі задач і виводить відсотки виконання всередині смужки. Плавна анімація пульсації активується під час виконання будь-якого із завдань. **Кнопка «🗑 Очистити»:** розміщена в шапці поруч із «▶ Запустити». Повністю очищує `jobs` і скидає `job_counter` до `0` — тобто наступна задача після очищення знову отримає `#1`. Кнопка заблокована (`add_enabled(false, ...)`) якщо хоча б одна задача в статусі `Running`, `AwaitingControl` або `AwaitingMediaControl`; при hover на заблокованій кнопці показується tooltip-пояснення.
...
- **Мультивіконна архітектура логів, контролю та чату:** Всі три типи допоміжних вікон підтримують одночасне відкриття для різних задач. Стан зберігається у трьох HashMap-полях `VideoMakerApp`:
  - `open_job_logs: HashMap<u64, String>` — відкриті вікна логів (job_id → job_name)
  - `open_job_controls: HashMap<u64, TranslationControlWindowState>` — відкриті вікна контролю перекладу
  - `open_agent_chats: HashMap<u64, AgentChatWindowState>` — відкриті вікна чату з агентом
  Клік по картці задачі використовує `entry().or_insert_with()` — якщо вікно вже відкрите, повторний клік не скидає введені дані. Кожен тип вікна рендериться у власному циклі: збираємо ключі у Vec, ітеруємо, закриваємо зачинені після циклу (щоб уникнути borrow конфлікту на HashMap). Кожне вікно отримує унікальний egui::Id через job_id.
- **Клікабельність задач та вікно логів:** Кожна картка в черзі є інтерактивною (`Sense::click()`). При наведенні курсор змінюється на `PointingHand`. Клік по картці вставляє job_id у `open_job_logs` та відкриває окреме спливаюче вікно `egui::Window` з детальними логами виключно цієї задачі. Одночасно може бути відкрито кілька вікон логів для різних задач.
- **Вкладка Галерея (`Tab::Gallery`):** відображає адаптивну сітку прев'ю медіафайлів (`jpg/jpeg/png/webp` + відео `mp4/webm/mov`) з папок `{task_dir}/media/` для всіх задач черги. Сітка будується через `ui.horizontal_wrapped()` — мініатюри автоматично переносяться на новий рядок залежно від ширини вікна. Кожна задача — розкривний `CollapsingState` з кількістю файлів у заголовку. Вкладка видима в навбарі лише якщо хоча б одна задача має непорожню `media/`. Якщо жодної задачі немає — `active_tab` скидається на `Main`. На вкладці Галерея черга (`queue_panel`) прихована щоб не займати місце. Якщо є задача у стані `AwaitingMediaControl` — у верхньому правому куті вкладки з'являється кнопка «Продовжити обробку», яка сигналізує condvar і відновлює пайплайн. Авто-перехід на Gallery: у `update()` при першому появленні задачі в стані `AwaitingMediaControl` виконується одноразовий автоматичний перехід `active_tab = Tab::Gallery`. ID задачі записується у `media_control_notified: HashSet<u64>` щоб уникнути примусового повернення при кожному кадрі — користувач може вільно перемикатись між вкладками після автоматичного переходу. Клік по картці задачі `AwaitingMediaControl` у черзі теж перемикає на Gallery. Текстури зображень кешуються у `gallery_textures: HashMap<PathBuf, Option<TextureHandle>>` — `None` означає "ще завантажується або помилка". Завантаження **асинхронне**: при першому рендері картки шлях потрапляє у `image_load_requests`, після відмалювання фрейму запускається `start_image_loading` у фоновому потоці (читання файлу + декодування `image = "0.25"` → `ColorImage` → `ctx.load_texture`), результат накопичується у `gallery_image_result: Arc<Mutex<Vec<(PathBuf, Option<TextureHandle>)>>>` і дренується в `gallery_textures` на наступному тику. **Важливо:** до переходу на асинхронне завантаження `load_image_texture` викликався синхронно у циклі рендерингу — 50+ зображень блокували UI на кілька секунд. Відеофайли (`mp4/webm/mov`) відображаються з першим кадром як мініатюра — вилучається через `ffmpeg pipe` без тимчасових файлів; поки thumbnail завантажується — показується темний прямокутник 16:9 з ▶.
  ...
  - **Перегенерація зображення («з тими ж»):** зчитує оригінальний промт з `media/prompts.json`, тип медіа та пріоритет провайдерів зі знімку налаштувань задачі (`JobSettings`). Викликає `regenerate_single_media` у фоновому потоці. Система відстежує **одночасно кілька** перегенерацій: `media_regen_paths: Arc<Mutex<HashSet<PathBuf>>>` — набір усіх файлів що зараз перегенеровуються (спінери показуються скрізь де є цей файл: галерея, медіа-пул редактора, таймлінія); `media_regen_results_queue: Arc<Mutex<Vec<(PathBuf, Result<(), String>)>>>` — черга результатів, яку `app.rs` дренує кожен кадр через `drain(..)`. Окремо `media_regen_result: Arc<Mutex<Option<...>>>` — слот виключно для вікна кастомної перегенерації (відображення помилки). **Після успіху** з черги `app.rs` виконує чотири дії: (1) `gallery_textures.remove(path)` — скидає текстуру у Галереї; (2) якщо відкритий редактор монтажу — видаляє `cache_dir` з диску, очищає `FrameCache` через `clear_for_media_id`, перестворює `MediaItem` у пулі зі збереженням `id`; (3) виставляє `editor.preview_stale_path = Some(path)` якщо відкрите fullscreen-прев'ю цього файлу — замість негайного скидання текстури (дає GPU-бекенду кадр на звільнення старої текстури перед завантаженням нової; без цього «половина по горизонталі сіра» при різних розмірах); (4) скидає `media_regen_paths` — спінер зникає. Все разом гарантує що нове зображення з'являється у всіх місцях UI без артефактів.
  ...
  - **Нові поля стану:** `gallery_preview`, `media_regen_loading`, `media_regen_result`, `media_regen_target`, `media_regen_paths`, `media_regen_results_queue`, `media_regen_window_open`, `media_regen_base_settings`, `media_regen_media_type`, `media_regen_image_priority`, `media_regen_video_priority`, `media_regen_prompt`, `media_regen_error`, `media_regen_job_id`, `media_regen_job_name`, `gallery_prompt_popup`. Відео: `video_thumbnails`, `video_thumb_loading`, `video_thumb_result`, `video_hover_frames`, `video_hover_state`, `video_hover_loading`, `video_hover_result`, `video_player`. Асинхронне завантаження зображень: `gallery_image_loading: Arc<Mutex<HashSet<PathBuf>>>` (файли у процесі завантаження), `gallery_image_result: Arc<Mutex<Vec<(PathBuf, Option<TextureHandle>)>>>` (готові результати).
- **Клікабельність задач та вікно контролю перекладу:** Якщо задача перебуває у статусі `AwaitingControl` (Очікує контролю), клік по її картці у черзі відкриває вікно «Контроль перекладу». Одночасно може бути відкрито кілька таких вікон для різних задач. У ньому можна перевірити та відредагувати отриманий переклад перед запуском наступних етапів. Між текстовим полем та кнопками відображається рядок статистики перекладеного тексту (символи • слова • токени) — оновлюється в реальному часі. Якщо переклад виконувався через OpenRouter — поруч зі статистикою відображається точна вартість у форматі `$0.00000`. **Заголовок вікна** показує назву задачі у форматі «Контроль перекладу — Назва задачі» (замість безіменного номера), щоб не загубитись при кількох задачах одночасно.
...
- **`current_pipeline_template()`** — метод `impl VideoMakerApp`, збирає поточний стан усіх налаштувань пайплайну у знімок `PipelineTemplate`. Використовується при записі в `task_history` та при відновленні задач з history без шаблону.
- **`enqueue_from_history(entry: &TaskHistoryEntry)`** — метод `impl VideoMakerApp`, викликається при кліку на картку в панелі историї. Джерело налаштувань визначається за `entry.template_name`: якщо `Some(name)` — завантажує `PipelineTemplate` з файлу шаблону через `load_template(name)`; якщо `None` — бере поточний стан панелі через `current_pipeline_template()`. З `entry` завжди беруться тільки `text` та `name`. `voicebot_key` та `googler_key` беруться з поточного стану програми (їх нема в `PipelineTemplate`). Якщо в папці задачі вже є файли — відкриває стандартний діалог відновлення (`resume_dialog_open`, `resume_pending`); якщо папка чиста — одразу додає в чергу.
...
**Клік по картці** одразу додає задачу в чергу через `enqueue_from_history` — налаштування панелі пайплайну **не змінюються**. Джерело налаштувань: якщо задача мала шаблон (`entry.template_name = Some("назва")`) — завантажуються налаштування з цього шаблону; якщо `null` — беруться поточні налаштування з панелі пайплайну. З `entry` завжди беруться тільки `text` (текст сценарію) та `name` (назва задачі). Якщо папка задачі вже містить проміжні файли — замість прямого додавання відкривається стандартний діалог відновлення. `draw_task_history_panel` повертає `Option<TaskHistoryEntry>`.

**`TaskHistoryEntry`** більше **не зберігає** знімок `PipelineTemplate` у JSON — лише 5 булевих (`stage_translation`, `stage_voiceover`, `stage_video`, `stage_subtitles`, `stage_editing`) для відображення кольорових крапок. Старі записи з полем `settings` у `task_history.json` читаються без помилок — serde ігнорує невідоме поле.
...
- **Прев'ю + транспортні кнопки** (центр) — відображає кадр на поточній позиції плейхеду з ефектами зуму, покачування, переходами та overlay-доріжками. Підтримує адаптивну якість: під час playback/скрабінгу показує легкий scrub-кадр, після зупинки плейхеду — автоматично підтягує чіткий still-кадр. **Базова доріжка (V1, `track_idx=0`)** рендерується першою з переходами. Якщо два кліпи накладаються на одній доріжці — автоматично показується crossfade (плавне згасання/проявлення) незалежно від налаштувань пайплайну. Поверх неї рендеруються всі overlay-кліпи (`track_idx≥1`) з їхніми `scale`/`pos_x`/`pos_y` трансформами. Транспортні кнопки ⏮ ⏹ ▶/⏸ ⏭ розташовані **під відео**, над таймлінією. При відтворенні паралельно програється аудіо-трек (`voice.wav`/`voice.mp3`). Прев'ю центрується вручну через `pad_x = (avail_w - frame_w) / 2.0` (без `vertical_centered` — він спричиняв дублювання кнопок). Клік/drag на прев'ю активує трансформ виділеного overlay-кліпу через `PreviewDragState`.
- **Інспектор** (праворуч від прев'ю) — показується при виборі кліпу на таймлінії. Містить три секції: **«Медіа»** — назва файлу, тривалість, кнопка «Скинути»; **«Трансформ»** — повзунки `Scale`/`Pos X`/`Pos Y` + кнопка `Reset` (тільки для overlay-доріжок `track_idx≥1`) + слайдер **«Прозорість»** (`opacity` 0.0–1.0); **«Ефекти»** — чекбокси «Зум» і «Покачування» (тільки для `ClipKind::Image`). Позиція і масштаб також оновлюються drag-взаємодією безпосередньо на прев'ю.
- **Таймлінія** (внизу) — горизонтальний ScrollArea з лінійкою; N доріжок (відео V1, V2, … та аудіо A1, A2, …) + опційна голосова доріжка `♪ Voice`; кольорові блоки (синій=відео, фіолетовий=зображення, зелений=аудіо); лінія відтворення; повна підтримка drag. У шапці таймлінії (над доріжками) розміщені дві кнопки: **«+ Відео»** (додає відеодоріжку зверху, зсуваючи всі кліпи вниз) та **«+ Аудіо»** (додає аудіодоріжку знизу). Кожна доріжка має тип (`TrackKind::Video` / `TrackKind::Audio`) і відповідний лейбл (V1/V2… або A1/A2…). Голосова доріжка `♪ Voice` інтерлівінгується серед звичайних — її позиція (`voiceover_track_idx`) змінюється вертикальним перетягуванням. **Зони накладання:** при перетягуванні кліпа на інший на тій самій доріжці з'являється жовта діагональна сітка (crosshatch, `rgba(255,182,72,…)`), що позначає область переходу. У правій частині зони — **кнопка ◈**, при натисканні відкриває випадаючий список з 49 xfade-переходів FFmpeg (fade, wipeleft, dissolve, circleopen, diagtl, …). Список має обмежену висоту з прокруткою (не вилазить за межі екрану), відкривається по `primary_pressed` (без конфлікту з подвійним кліком відкриття медіа), закривається по Esc або вибору переходу. Обраний перехід зберігається в `overlap_transition` incoming-кліпу та одразу записується у `timeline.json`. **Смужка прозорості:** всередині кожного кліпу є біла горизонтальна лінія (3px); її вертикальна позиція відображає opacity — вгорі = 1.0, внизу = 0.0. Перетягуванням смужки вгору/вниз плавно змінюється прозорість; також доступна колір-заливка самого блоку кліпу (alpha тла залежить від opacity). При opacity drag `clip_drag_state` блокується і навпаки.

**Магнітний снап (snap-to-edge):** при горизонтальному перетягуванні кліп автоматично прилипає до країв інших кліпів на тій же доріжці (стиковка впритул) та до плейхеда. Поріг спрацювання ~10px у просторі таймлінії. При активному снапі з'являється **жовта пунктирна вертикальна лінія** через усю висоту доріжок — напівпрозора (2px) зі штрихами (крок 8px). Якщо протягнути трохи далі за поріг — снап зривається і кліпи починають накладатись (overlap). Снап працює лише в межах однієї доріжки; кліпи на інших доріжках не впливають.
- **Топбар** (зверху) — повзунок масштабу таймлінії (`timeline_zoom`), налаштування якості/FPS превʼю («Превʼю:» + два drop-down: якість + FPS), лічильник кліпів і тривалості, дві кнопки вибору рендеру: **«🎬 FFmpeg»** (зелена) та **«✂️ CapCut»** (синя), а також кнопка розгортання на весь екран / згортання у вікно (**`🗖`** / **`🗗`**). Налаштування превʼю зберігаються у `settings.json` і не впливають на фінальний рендер.
...
**Система кадрового превʼю (pre-extraction + LRU disk cache + adaptive quality):**
...
- **`MediaItem::new(path, cache_base, preview_render)`** — створює елемент пулу і стартує фонове витягування. Папки кешу тепер містять версію якості/FPS у назві (наприклад, `{hash}_bal_w960_q4_f30`). При зміні якості у топбарі створюється новий кеш, старий ігнорується.
- **Адаптивна якість:** кадри витягуються одразу у двох якостях:
  - **Scrub-кеш** — для швидкого перетягування/playback. Залежить від обраної якості (Performance 640px, Balanced 960px, High 1280px, Ultra 1600px) та FPS (15/30/60).
  - **Sharp-кеш** — для чіткого still-кадру. Генерується **не для всього відео**, а тільки для позицій, де користувач зупиняв плейхед. Роздільність: 1280–2560px залежно від профілю.
- **Версіонування кешу:** `frame_cache_dir()` додає тег якості + FPS у назву папки. При оновленні програми з новими параметрами старий кеш не використовується.
- **`.complete` маркер** — файл-флаг що витягування завершено. При повторному відкритті редактора — перевіряється першим, повторне витягування не відбувається. **Інвалідація при перегенерації:** після успішної перегенерації медіафайлу `app.rs` видаляє всю `cache_dir` через `std::fs::remove_dir_all` — разом із `.complete` та `000001.jpg`. `MediaItem` у пулі перестворюється (з тим самим `id`), що запускає нову екстракцію оновленого файлу. Без цього кроку редактор показував би старий кешований кадр навіть після заміни файлу на диску. **Аналогічну інвалідацію** виконує `refresh_placeholder_clips` (`state.rs`) при заміні стокового медіа через контекстне меню — файл зберігається за тим самим шляхом, тому без явного видалення кешу `.complete` блокувала б повторну екстракцію.
...
- **`FrameCache::get_frame(ctx, media, time, sharp_when_idle, settings)`** — LRU кеш (за замовчуванням 80 текстур). Ключ: `{media_id}_s_{frame_idx:06}` (scrub) або `{media_id}_h_{frame_idx:06}` (sharp). **Повністю неблокуючий**: якщо кадру нема в кеші, UI не декодує JPEG синхронно — ставить його в фонову чергу (до 3 паралельних завантажень) і одразу повертає найближчий кадр як fallback. При LRU hit тихо префетчить 2 наступні кадри. При `sharp_when_idle=true` (плейхед не рухається) генерує чіткий still-кадр через `generate_sharp_frame` — однокадровий ffmpeg з seek. Для зображень sharp-кадр створюється через `image` crate. Ключі з префіксом `s_` (scrub) і `h_` (sharp) живуть в одному LRU, що гарантує коректний fallback без стрибків між якостями. **Фолбек двонаправлений:** `cached_near_frame` шукає найближчий кадр в обидва боки (`|idx - frame_idx| ≤ max_distance`, де max_distance ≈ fps/3). При скрабінгу назад кешовані кадри виявляються «попереду» поточної позиції — без двонаправленого пошуку повертався б `None` і превʼю блимало.
...
- **Переходи (11 груп xfade + overlap-кросфейд):** два типи переходів. **Налаштовані:** визначаються через `clip_offset = t - clip_start_secs` відносно `transition_duration` (JobSettings). 11 груп xfade реалізовано вручну через UV/alpha/sub-rect. **Overlap-кросфейд (per-clip):** коли кліпи накладаються на таймлінії, перехід береться з поля `overlap_transition` incoming-кліпу (обраного через кнопку ◈ на жовтій зоні). Підтримуються всі 49 xfade FFmpeg — кожен мапується до одного з 11 графічних типів через `transition_kind()`. Тривалість переходу в прев'ю = фактична довжина накладання, а не глобальний `transition_duration`.
...
pub struct ClipDragState {
    ...
    pub snap_line_secs: Option<f32>,  // позиція лінії снапу для візуального індикатора
}
```
...
- **Магнітний снеп** — при русі порівнює лівий/правий край поточного кліпу з лівими/правими краями кліпів на тій же доріжці + плейхедом. Поріг: `10.0 / zoom` секунд (~10px). При спрацюванні встановлює `snap_line_secs` — позицію на таймлінії де краї збігаються, що використовується для малювання жовтої пунктирної лінії через `painter` у ScrollArea.
...
- **Загальні поля:** `total_duration_secs` (загальна тривалість відео), `audio_start_secs` (зсув оригінальної озвучки у секундах, за замовчуванням `0.0`), `voiceover_track_idx` (позиція голосової доріжки: `0` = перед V1, `num_tracks` = знизу). Кожен сегмент має поле `overlap_transition` — тип xfade-переходу для overlap-зони ("fade", "wipeleft", "dissolve", … 49 варіантів; за замовчуванням "fade").
...
**Відновлення null-медіа при завантаженні (`load_timeline_clips`):** якщо сегмент у `timeline.json` має `"media": null` і водночас є поле `"text"` (ознака pipeline-формату від агента, не редакторські gap-сегменти), `load_timeline_clips` намагається відновити медіашлях у двох режимах:
1. **Режим стоків** — якщо `stock_cache.json` існує у папці задачі, це єдине джерело медіа. Сегмент отримує медіа з `cache[i].selected.filename` якщо воно є на диску, або залишається плейсхолдером якщо ще не вибрано. Fallback-індексний маппінг в цьому режимі **не використовується** — щоб уникнути ситуації «всі сегменти отримують перший файл» при 1 завантаженому стоковому файлі.
2. **Звичайний режим** (без `stock_cache.json`) — пропорційний індексний маппінг: сортує файли `media/` за іменем і призначає `media/files[floor(i * n_files / n_segs)]`.

**«🎬 FFmpeg» / «✂️ CapCut»** — замість однієї кнопки «Продовжити рендер» є два варіанти. Доступні лише коли задача в стані `AwaitingMontageControl`. При кліку на будь-яку: 1) встановлюється `capcut_mode_override` у `PipelineJob` (`Some(false)` = FFmpeg, `Some(true)` = CapCut) — перевизначає налаштування `capcut_enabled` з панелі пайплайну; 2) `save_to_timeline()` записує зміни та логує результат; 3) сигналізує `montage_control_resume` condvar — пайплайн продовжується і читає override перед рендером; 4) вікно редактора **закривається автоматично**. Це дозволяє обирати між FFmpeg та CapCut безпосередньо в редакторі незалежно від того що виставлено в налаштуваннях.

**`draw_montage_editor_window` повертає `MontageEditorActions`** — структуру з чотирма полями: `animate_paths: Vec<PathBuf>` (зображення для оживлення image-to-video), `regen_action: Option<(file, settings, is_custom, job_id, job_name)>` (запит перегенерації медіа), `open_stock_picker: Option<usize>` (запит відкриття Stock Picker), `preview_render_changed: Option<PreviewRenderSettings>` (нові налаштування якості/FPS превʼю, якщо користувач змінив їх у топбарі — `app.rs` записує у `self.preview_quality`/`self.preview_fps` і тригерить автозбереження).
...
- **Зсув та позиція оригінального аудіо:** Оригінальний звуковий файл озвучки (`voice.wav`/`voice.mp3`) можна переміщувати **горизонтально** (зсув за часом, змінює `audio_start_secs`) та **вертикально** (між доріжками, змінює `voiceover_track_idx`) — обидва значення зберігаються у `timeline.json`. Під час вертикального перетягування позиція голосової доріжки визначається за координатою миші й оновлюється в реальному часі.
...
Вікно вибору стокового медіа (Pexels або Pixabay) для одного сегмента. Відкривається **тільки з редактора монтажу** — кліком на плейсхолдер-кліп або через контекстне меню.

**Архітектурна зміна:** Пікер працює виключно у режимі single-mode, не пов'язаний безпосередньо з condvar-ами пайплайну і не блокує інтерфейс модальними overlay. Вибір підтверджується через «▶ Продовжити рендер» у редакторі монтажу.

**Вибір кількох сервісів одночасно:**
- Інтегровано випадаючий/згортаємий фільтр `🌐 Сервіси` (`show_services_filter: bool`), що дозволяє вибрати пошук по Pexels, Pixabay або обох сервісах одночасно.
- За замовчуванням при відкритті увімкнені всі сервіси, для яких вказані API-ключі. Якщо жодного ключа не додано, користувач отримує попередження `stock_key_missing_warning`.

**Паралельний пошук та групування:**
- Фоновий пошук (`trigger_segment_search`) виконується паралельно в окремих потоках для кожного обраного провайдера.
- Результати позначаються відповідним сервісом (`provider`) та групуються у сітці під заголовками на кшталт `🎬 Pexels Videos` / `🎬 Pixabay Videos` або `📷 Pexels Photos` / `📷 Pixabay Photos`.

**Покращення UX та продуктивності (запобігання фрізам):**
- **Асинхронне декодування мініатюр:** Щоб уникнути зависання головного UI-потоку (візуальні фрізи при відображенні результатів), декодування зображень за допомогою `image::load_from_memory` повністю перенесено з UI-потоку в background-потоки завантаження. Вони повертають вже готовий об'єкт `egui::ColorImage`, а головний потік лише реєструє текстури.
- **Запобігання виходу вікон за межі екрана та авто-розширення**:
  - Для вікон `stock_picker_single` та `stock_trim_editor` додано примусове позиціонування `.constrain(true)` та лімітування максимального розміру `.max_size()`, що утримує їх у межах екрана програми.
  - Додано динамічне обмеження висоти ScrollArea через `.max_height(screen.height() - 200.0)` у вікні стоків, що змушує її вчасно показувати вертикальний скролбар і не дає вікну роздуватися під статус-бар програми.
  - Розрахунок кількості колонок `cols` для `egui::Grid` здійснюється динамічно на основі `ui.available_width()`, оскільки вертикальна `ScrollArea` (без горизонтального скролу) запобігає feedback-петлі автоматичного розширення. Це дозволяє вікну плавно стискатися до мінімального розміру (420px), адаптуючи сітку під ширину вікна.
  - Великий прев'ю-кадр у вікні нарізки `stock_trim_editor` тепер адаптивно обмежує свою висоту (`avail_h - 180.0`), запобігаючи вертикальному переповненню та роздуванню вікна нарізки.
...
- `StockPickerState.pexels_enabled` + `StockPickerState.pixabay_enabled` — стан чекбоксів для кожного провайдера.
...
- `flush_segment_search(state)` — викликається кожен кадр у `draw_stock_picker`.

**Вибір типу медіа:**
Кожен сегмент може мати і фото, і відео. За замовчуванням першим у перемикачі йде розділ **🎬 Відео**, другим — **📷 Фото**. За замовчуванням (`show_videos = true`) відкриваються відео.
...
- **Великий превью (16:9)** — анімований playback обраного сегменту. Максимальна ширина 820px,高度 до 420px.
...
- **Перемикання відео без закриття:** якщо trim editor відкритий і користувач клікає на інше відео у сітці — trim editor автоматично закривається (`state.trim_edit = None`) і починається завантаження нового відео. Умова `state.trim_edit.is_none()` навмисно прибрана з перевірки кліку в `draw_video_grid`; `start_video_download_if_needed` скидає `trim_edit` перед стартом нового завантаження.
...
- `MontageEditorState.needs_stock_refresh: bool` — після вибору медіа → `refresh_placeholder_clips` замінює кліп реальним файлом, копіює `SelectedMedia.trim_start` у `EditorClip.trim_start` та зберігає `seg_idx` у `EditorClip.stock_seg_idx`. Повертає `true` якщо файл ще завантажується (треба повторити наступного кадру). **Важливо:** функція обробляє **два** випадки: плейсхолдери (`is_placeholder = true`, індекс сегмента береться з `media_id = "placeholder_N"`) і вже-призначені стокові кліпи (`is_placeholder = false`, індекс береться з `stock_seg_idx`) — без цього заміна через контекстне меню не відображалась на таймлайні.
...
**Інвалідація кешу при заміні стокового медіа:** файл нового відео зберігається за тим самим шляхом `{seg_idx+1:04}.mp4`, тобто `path_hash` збігається зі старим. Якщо в `.frame_cache/{hash}/` вже є маркер `.complete` — `MediaItem::new` знайде його і пропустить повторну екстракцію, показуючи кадри старого відео. Тому `refresh_placeholder_clips` перед кожним `MediaItem::new` видаляє стару директорію кешу через `std::fs::remove_dir_all(&old_cache)` і одразу видаляє старий запис з `media_pool` — так нова екстракція стартує з чистого стану і прев'ю відображає правильне відео.
...
- **`JobStatus`** — `Pending → Running / AwaitingControl (пауза для контролю перекладу) / AwaitingMediaControl (пауза для перегляду зображень) / AwaitingMontageControl (пауза перед монтажем для редактора) / AwaitingAgentControl (пауза після роботи агента — очікує натискання «✓ Підтвердити» у вікні чату) → Done / Failed(String)`. Обгорнутий у `Arc<Mutex<T>>`, бо змінюється з фонового потоку. `AwaitingMontageControl` відображається зеленим, `AwaitingAgentControl` — синім.
...
- **`skip_agent_on_resume: bool`** у `JobSettings` — якщо `true`, при `RetryStage::Video` агент (`run_agent_timeline`) не запускається — `timeline.json` вже є і зберігається. Встановлюється в `enqueue_with_resume` коли `keep_timeline = true` у діалозі відновлення. За замовчуванням `false`.
- **`skip_existing_media: bool`** у `JobSettings` — режим догенерації відсутніх медіафайлів. Якщо `true`, перед кожним spawn-потоком генерації перевіряється `media_file_exists_by_index(media_dir, i+1)` — якщо файл `0001.jpg/mp4/...` вже є, він пропускається без запиту до API і прогрес інкрементується. Встановлюється лише `enqueue_fill_missing` (кнопка «Догенерувати відсутні» у діалозі відновлення). За замовчуванням `false`.
...
- **`timeline_rebuild_requested: Arc<Mutex<bool>>`** у `PipelineJob` — прапор що встановлюється кнопкою «Перебудувати таймлінію» у вікні чату з агентом. `app.rs` перевіряє цей прапор кожен кадр: якщо `true` — скидає в `false`, запускає `find_changed_prompts_for_rebuild` для виявлення змінених промтів, перегенеровує тільки ці медіафайли, і перезавантажує `MontageEditorState::load`.
- **`montage_control_resume: Arc<(Mutex<bool>, Condvar)>`** у `PipelineJob` — примітив для відновлення після редактора монтажу. UI сигналізує через кнопки «🎬 FFmpeg» або «✂️ CapCut» у вікні `draw_montage_editor_window`.
- **`capcut_mode_override: Arc<Mutex<Option<bool>>>`** у `PipelineJob` — перевизначення режиму монтажу з редактора. `None` = використати `settings.capcut_enabled` (з налаштувань на момент додавання задачі). `Some(false)` = примусовий FFmpeg, `Some(true)` = примусовий CapCut. Встановлюється кнопками топбара редактора монтажу перед resume. `run_pipeline` та `run_final_stages` читають це поле через `capcut_mode_override.lock().unwrap().unwrap_or(settings.capcut_enabled)` — так вибір у редакторі завжди виграє над початковими налаштуваннями.
...
- **`agent_control_resume: Arc<(Mutex<bool>, Condvar)>`** у `PipelineJob` — condvar для відновлення пайплайну після паузи `AwaitingAgentControl`. UI сигналізує через кнопку «✓ Підтвердити» у вікні чату з агентом.
- **`agent_session: Arc<Mutex<Option<AgentSessionInfo>>>`** — активна сесія агента (`session_id`, `service`, `model`). Встановлюється **ДО** запуску агента — завдяки цьому session_id доступний для `--resume` навіть якщо агент завершився з помилкою (наприклад, quota limit). `None` — сесія ще не стартувала.
...
- **Video** — спавнить потік: у агентному режимі (`Claude Code` / `Gemini CLI` / `Codex CLI`) **і якщо `!settings.skip_agent_on_resume`** — спочатку викликає `run_agent_timeline` для створення `timeline.json`, потім `run_video_branch`, потім `assign_media_to_timeline` → media-control пауза → `run_final_stages`. Якщо `skip_agent_on_resume = true` (відновлення з наявним `timeline.json`) — агент пропускається, одразу `run_video_branch`. У звичайному режимі (не агент) — одразу `run_video_branch` → пауза → `run_final_stages`.
...
**`run_final_stages`** (`mod.rs`) — хелпер для спільної фінальної частини: `build_timeline` (якщо `video_enabled` **і не агентний режим** (`Claude Code` / `Gemini CLI` / `Codex CLI` / `AGY CLI`) — в агентному `timeline.json` вже створений агентом і перезаписувати не можна) → **пауза контролю монтажу** (якщо `montage_enabled && montage_control_enabled`) → або `generate_capcut_project`, або `run_montage`. Вибір між ними: `capcut_mode_override.lock().unwrap().unwrap_or(settings.capcut_enabled)` — спочатку перевіряється override з редактора, якщо `None` — значення з налаштувань задачі. Ці два шляхи взаємовиключні: якщо CapCut обрано, FFmpeg-монтаж пропускається. Пауза контролю монтажу: встановлює статус `AwaitingMontageControl`, блокується на `Condvar::wait()`, після сигналу повертається в `Running`. Приймає `&status`, `&montage_control_resume` та `&capcut_mode_override` — ті самі Arc що й у `run_pipeline`.
...
- **`call_llm`** (`src/core/llm.rs`) — єдина точка виклику будь-якого LLM-сервісу. Будує `user_content` з промту + тексту (підстановка `{{text}}`), диспетчеризує на відповідний сервіс (`Claude Code` → `call_claude_code`, `Gemini CLI` → `call_gemini_cli`, `Codex CLI` → `call_codex`, `AGY CLI` → `call_agy_cli`, інше → `call_openrouter`). Приймає `working_dir: Option<&str>`: для CLI-агентів передається `Some(&settings.save_path)`, щоб агент запускався відносно кінцевої папки задачі; для OpenRouter ігнорується. **`allow_tools: bool`** — передається `true` лише з `run_agent_timeline`; при `true` для Claude Code додається `--allowedTools Bash,Write,Read`, що дозволяє агенту записувати файли (без цього `-p` mode блокує інструменти). Для Gemini CLI та AGY CLI параметр ігнорується — вони використовують `--dangerously-skip-permissions` за замовчуванням. Повертає `Result<(String, Option<f64>), String>` — текст та вартість (вартість заповнюється тільки для OpenRouter). Використовується і для перекладу, і для генерації відеопромтів, і для агентного таймлайну.
...
Якщо у секції Відеоряд обрано `Claude Code`, `Gemini CLI`, `Codex CLI` або `AGY CLI`, пайплайн перемикається в агентний режим:
...
2. **`run_agent_timeline`** — читає `subtitle.srt`, підставляє вміст у `{{srt}}` і шлях до `timeline.json` у `{{path}}`. Завжди використовує стримінговий виклик `call_agent_new_session_streaming`: генерує `uuid_v4()`, одразу додає в `agent_chat` початкове повідомлення з аргументами запуску (як в логах), потім отримує відповідь по чанках через callback `on_chunk`, який аппендить до останнього повідомлення і викликає `ctx.request_repaint()`. Після завершення зберігає `agent_chat.json` та `AgentSessionInfo` (session_id, service, model) — завжди, для можливості подальшого `--resume` з вікна чату. Пайплайн **не зупиняється** після агента — одразу продовжує до `run_video_branch`.
3. **`run_video_branch`** — зчитує сегменти вже зі створеного агентом `timeline.json` (поле `text`) і відправляє їх напряму до Googler API без будь-якої підстановки в `video_prompt`. LLM-генерація промтів також пропускається. Поле `text` агент пише як опис сцени — і він же стає промтом для генерації медіа.
...
```
`media: null` обов'язкове — програма заповнює після генерації. `text` — опис сцени для Googler (не транскрипція): відправляється напряму як промт для генерації медіа без будь-якої підстановки у `video_prompt`.

**Кнопка 💬 та вікно чату з агентом:**

Кнопка 💬 з'являється у картці задачі (поруч з ✂ редактором) коли обрано `Claude Code`, `Gemini CLI`, `Codex CLI` або `AGY CLI` як сервіс відеоряду — незалежно від `agent_control_enabled`. Дозволяє відкрити чат у будь-який момент, щоб спостерігати за роботою агента в реальному часі.
...
- **Поле введення** (Enter = відправити, Shift+Enter = новий рядок) і кнопка «Надіслати» — доступні лише якщо є збережена сесія (`agent_session` не `None`). Оскільки сесія встановлюється **до** запуску агента, кнопка стає доступною навіть якщо агент завершився з помилкою.
- **Кнопка «✓ Підтвердити»** — завжди видима у вікні чату. Активна (зелена) тільки коли статус задачі `AwaitingAgentControl`. При кліку — відправляє сигнал через condvar (`agent_control_resume`), пайплайн відновлює виконання. Неактивна (сіра) в усіх інших станах.
- **Кнопка «Перебудувати таймлінію»** — завжди видима (незалежно від наявності сесії). Встановлює прапор `timeline_rebuild_requested = true`. `app.rs` перехоплює прапор кожен UI-кадр, порівнює поточні тексти сегментів `timeline.json` з `media/segment_texts.json`, запускає фонову перегенерацію змінених медіафайлів зі спінером у галереї та редакторі, і перезавантажує `MontageEditorState`.
- **AGY CLI та чат:** AGY є TUI-застосунком що пише у Windows Console API напряму (WriteConsole/WriteConsoleW), обходячи stdout/stderr pipe. `Stdio::piped()` не перехоплює цей вивід — у вікні чату відповідь може бути порожньою. Єдиний надійний спосіб отримати вивід — ConPTY (pseudo-terminal), але у поточній реалізації використовується простий `cmd.output()` без PTY.

**Взаємодія з агентом та відновлення пайплайну:**

Після завершення (або помилки) `run_agent_timeline` пайплайн **автоматично переходить в режим паузи `AwaitingAgentControl`** і чекає на підтвердження користувача:

1. Агент завершив роботу (або зупинився з помилкою — quota limit тощо). Статус задачі: **Очікує підтвердження агента** (синій).
2. Відкрити вікно чату 💬: якщо потрібна корекція — написати агенту «continue» або іншу команду, агент продовжить і відредагує `timeline.json`.
3. Якщо потрібна перебудова медіа — натиснути «🔄 Перебудувати таймлінію»: `timeline_rebuild_requested = true` → `app.rs` виявляє змінені сегменти та перегенеровує їх.
4. Натиснути **«✓ Підтвердити»** — пайплайн відновлюється, продовжуючи з наступного кроку після агента.

Якщо агент завершився з помилкою (наприклад quota limit): сесія вже збережена (встановлена **до** запуску), тому кнопка «Надіслати» активна — можна написати агенту і він продовжить у тій самій сесії (`--resume`).

Чат зберігається у `{task_dir}/agent_chat.json`.
...
**Збереження промтів (`media/prompts.json` та `media/segment_texts.json`):** після генерації всіх промтів (Phase 1) зберігаються два файли:
- `media/prompts.json` — фінальні повністю стилізовані промти (після підстановки `video_style_prompt.replace("{{text}}", text)`). Використовується Галереєю для перегенерації окремого файлу. `read_prompt_for_file(file_path)` парсить ім'я файлу (`"0003.jpg"` → індекс `2`) і повертає відповідний елемент.
- `media/segment_texts.json` — сирі тексти сегментів від агента (до накладення стилю). Використовується при перебудові таймлінії: порівнює нові тексти з `timeline.json` проти збережених, щоб знайти ті, що агент змінив. **Важливо:** оновлюється синхронно при виявленні змін (до завершення перегенерації) — тому кілька послідовних перебудов одного сегменту завжди правильно порівнюються із попереднім станом.

**`pub fn regenerate_single_media`** (`core/pipeline/mod.rs`) — публічна функція для перегенерації одного медіафайлу. Запускається у `std::thread::spawn`. Зчитує промт з `media/prompts.json` (або використовує переданий `custom_prompt`), генерує медіа через `generate_image_with_priority` / `generate_video_with_priority`, зберігає результат через `save_media_bytes` (підтримує data URI та HTTP URL). Пише лог через `log_job(job_id, &job_name, ...)`. Результат (`Ok(())` або помилка) → `result_slot: Arc<Mutex<Option<Result<(), String>>>>`. Прапор `loading: Arc<Mutex<bool>>` встановлюється/знімається навколо виклику. Необов'язковий параметр `path_loading_set: Option<Arc<Mutex<HashSet<PathBuf>>>>` — якщо передано `Some(set)`, файл додається до цього набору перед генерацією і видаляється після. Використовується при масовій перегенерації після перебудови таймлінії для показу спінера в галереї та редакторі монтажу. `ctx.request_repaint()` ініціює перемальовування після завершення.

**`pub fn find_changed_prompts_for_rebuild`** (`core/pipeline/mod.rs`) — порівнює поточні тексти сегментів `timeline.json` з збереженими у `media/segment_texts.json`. Для кожного зміненого сегменту читає **реальний шлях медіа з поля `"media"` сегменту** — якщо поле `null` (агент не зберіг шляхи), сегмент пропускається і перегенерація **не запускається**. Якщо шлях є і файл існує — будує повний промт через `video_style_prompt.replace("{{text}}", new_text)`. Повертає `Vec<(PathBuf, String)>` — пари (файл, промт) для перегенерації. **Важливо:** раніше використовував сортований індексний маппінг (`files[floor(i*n/segs)]`) незалежно від поля `media`, що в stocks-режимі з 1 завантаженим файлом призводило до регенерації всіх сегментів з тим самим файлом. Відразу після порівняння оновлює `segment_texts.json` з новими текстами (до запуску фонових потоків перегенерації).

**`pub fn animate_single_image`** (`core/pipeline/mod.rs`) — анімує одне зображення (image-to-video). Запускається у `std::thread::spawn`. Читає файл, кодує у base64 data URI з правильним mime-типом (`image/jpeg` / `image/png` / `image/webp`), викликає `animate_image_with_priority` (3 спроби на провайдера, 5s затримка між провайдерами). Промт береться з `media/prompts.json` через `read_prompt_for_file` (той самий промт що й при генерації зображення); якщо промт порожній — fallback `"Animate this image with smooth, natural motion."`. Після отримання відповіді логує ім'я провайдера: `"Animate {file}: провайдер — {provider}"`. Результат-data URI зберігається поряд з оригіналом через `save_media_bytes`, оригінальне зображення видаляється через `std::fs::remove_file`. Вставляє/видаляє `file_path` у `loading_set: Arc<Mutex<HashSet<PathBuf>>>` і викликає `ctx.request_repaint()` до і після генерації.
...
- **Глобальне лімітування запитів OpenRouter (`OpenRouterLimiter`), Claude Code (`ClaudeLimiter`), Gemini CLI (`GeminiLimiter`), Codex CLI (`CodexLimiter`), AGY CLI (`AgyLimiter`), Edge TTS (`EdgeTTSLimiter`), VoiceBot (`VoiceBotLimiter`), AssemblyAI (`AssemblyAILimiter`), FFmpeg (`FfmpegLimiter`), Googler зображення (`GooglerImageLimiter`) та Googler відео (`GooglerVideoLimiter`):**
  ...
  - Метод `acquire()` блокує потік, якщо досягнуто ліміту активних запитів, повертаючи відповідний дозвіл-перміт (`OpenRouterPermit<'a>`, `ClaudePermit<'a>`, `GeminiPermit<'a>`, `CodexPermit<'a>`, `AgyPermit<'a>`, `EdgeTTSPermit<'a>`, `VoiceBotPermit<'a>`, `FfmpegPermit<'a>`, `GooglerImagePermit<'a>`, `GooglerVideoPermit<'a>`).
...
| **Googler (баланс)** | `googler.fast-gen.ai/api/v3/account/usage` | `GooglerBalance`: img/video used+limit, threads active+allowed |
| **Googler v4 (image/video)** | `/api/v4/{provider}/image/from-text`, `/v4/{provider}/video/from-text` та ін. | Генерація зображень/відео; POST → отримуємо data URI або URL безпосередньо у відповіді |
| **Googler v5 (video)** | `POST /api/v5/generations` → `GET /api/v5/generations/{id}` → `GET storage.fast-gen.ai/{download_path}` | Провайдери: `flow_fast`, `flow_light`, `flow_quality`, `flow_omni_flash`; polling статусу до `"completed"` / `"failed"`. Анімація зображень (image-to-video) підтримується через поле `inputs: [data_uri]` для `flow_omni_flash`, `flow_fast`, `flow_light` (operation = `_from_ingredients`); `flow_quality` **не підтримує** анімацію. |
| **Googler (usage v5)** | `GET /api/v5/usage` | Залишок кредитів і ліміти (оновлена версія ендпоінту) |
...
- **Opacity кліпів у V1 (основна доріжка) не застосовується в FFmpeg-рендері.** Прозорість збережена в `timeline.json` і відображається у превʼю для всіх кліпів. Але у FFmpeg-пайплайні (`montage.rs`) opacity застосовується **тільки** до overlay-треків через `colorchannelmixer=aa=X` після `format=yuva420p`. Для V1 (основна відеодоріжка) opacity в FFmpeg не реалізована — там використовується xfade-ланцюжок, і вставка alpha-фільтра зламала б конкатенацію. Тому opacity=0.5 на V1-кліпі змінить вигляд тільки в превʼю редактора, не у фінальному відео.
...
- **Адаптивна якість превʼю (`FrameCache` + `PreviewRenderSettings`).** Під час перетягування плейхеду або playback використовується легкий scrub-кеш (менша роздільність, вищий JPEG qscale). Коли плейхед зупинився — у фоні генерується чіткий still-кадр. `FrameCache::get_frame` отримує параметр `sharp_when_idle` (true коли `!is_playing && !pointer_down && !in_transition`). Важливо: sharp-кадри не витягуються для всього відео — тільки для позицій, де користувач реально зупинився. При зміні якості/FPS через топбар весь поточний FrameCache перестворюється заново, а `MediaItem` перестворюються зі збереженням ID. Зміна якості превʼю **не впливає** на фінальний FFmpeg/CapCut рендер.

- **Налаштування превʼю зберігаються в `settings.json`.** Поля `preview_quality` (`"balanced"`, `"performance"`, `"high"`, `"ultra"`) та `preview_fps` (15/30/60) у `AppSettings` беруть участь у стандартному автозбереженні. Зміна з topbar-у редактора проходить ланцюгом: `draw_preview_settings` → `MontageEditorState.set_preview_render()` → `pending_preview_render` → `MontageEditorActions.preview_render_changed` → `app.rs` оновлює `self.preview_quality`/`self.preview_fps` → наступна autosave-перевірка записує на диск.
...
- **Скрабінг назад по таймлінії — `cached_near_frame` має бути двонаправленим.** При скрабінгу вліво кешовані кадри опиняються «попереду» поточної позиції (`idx > frame_idx`). Якщо пошук fallback-кадру ведеться тільки назад (`idx ≤ frame_idx`) — cache miss повертає `None` → превʼю миттєво блимає. Рішення: пошук за `|idx - frame_idx| ≤ max_distance` в обидва боки. Для playback-у це безпечно: при прямому відтворенні найближчим завжди виявляється попередній кадр (відстань 0–1), а не майбутній.
...
- **xfade переходи (per-pair, з пріоритетом редактора).** При накладанні кліпів на таймлінії FFmpeg-монтаж використовує фактичну тривалість overlap-зони та `overlap_transition` з редактора, **ігноруючи** глобальний `transition_duration` для цієї пари. Для пар без накладання — глобальні налаштування пайплайну (якщо `transition != "none"`). Фільтр-граф: overlap-пари склеюються `xfade` з per-pair параметрами, потім усі сегменти склеюються `concat`. Компенсація sync: кожен кліп подовжується на тривалість свого вихідного переходу (`adj_dur = orig_dur + overlap_dur`), offset для xfade = `sum(orig_dur[0..=k])` = `start_secs[k+1]`. Сумарна тривалість не збільшується.
...
- **`googler_video_disabled` — фільтрується при побудові `JobSettings`, не зберігається всередині нього.** `AppSettings` і `PipelineTemplate` зберігають повний список `googler_video_disabled` (щоб можна було увімкнути провайдера знову). При виклику `build_job_settings` у `pipeline/mod.rs` відеопріоритет фільтрується: `googler_video_priority.iter().filter(|p| !googler_video_disabled.contains(p)).collect()`. Результуючий `JobSettings.googler_video_priority` містить лише увімкнених провайдерів — вся подальша логіка пайплайну, перегенерацій та галереї автоматично поважає налаштування без жодних змін.

- **Googler v5: нескінченний retry при `rate_limit.concurrency_exceeded`.** HTTP 429 з тілом `{"code": "rate_limit.concurrency_exceeded", ...}` означає що сервер зайнятий — це тимчасовий стан, не справжня помилка. `try_generate_video` у таких випадках циклічно повторює запит із паузою 5s **без лічильника спроб** і без переходу до наступного провайдера. Усі інші HTTP 429 або 5xx обробляються стандартно: ліміт спроб + зміна провайдера.

- **Googler v5 — двостадійний процес (submit → poll → download).** `try_generate_video_v5` надсилає POST на `/api/v5/generations`, отримує `generation_id`, потім циклічно питає `GET /api/v5/generations/{id}` до статусу `"completed"` або `"failed"`. При `"completed"` — якщо є `download_path`, завантажує з `https://storage.fast-gen.ai/{download_path}`; якщо є `data` — декодує base64 безпосередньо. Відповідь v5 містить масив `results[0]` (для відео — завжди один елемент).
...
- **Додатковий кроп для `flow_omni_flash` (`is_omni`).** У провайдера `flow_omni_flash` водяний знак значно більший, ніж у інших. Тому `upscale_video_if_needed` приймає параметр `is_omni: bool`: якщо `true` — на початок FFmpeg vf-ланцюга **перед** основним кропом/апскейлом вставляється `crop=iw*0.89:ih*0.89:0:0` (обрізка 11% знизу та справа — top-left origin). Цей фільтр застосовується одним ffmpeg-проходом разом з апскейлом. Для всіх інших провайдерів поведінка незмінна. `generate_video_with_priority`, `generate_image_with_priority` та `animate_image_with_priority` повертають `(provider_name, data_uri)` — кортеж, щоб викликаючий код міг визначити `is_omni = provider == "flow_omni_flash"` і передати у `upscale_video_if_needed`. Лог генерації зображень та відео у `run_video_branch` і `regenerate_single_media` тепер містить ім'я провайдера.

- **`animate_single_image` використовує промт з `prompts.json`.** Анімація image-to-video бере той самий промт що й генерація оригінального зображення (`read_prompt_for_file` → індекс з імені файлу → елемент масиву `prompts.json`). Якщо промт порожній (файл відсутній або індекс не знайдено) — fallback `"Animate this image with smooth, natural motion."`. `prompts.json` не оновлюється при анімації — лишається з оригінальними промтами на випадок подальшої перегенерації зображень. Ендпоінти image-to-video: `flower → /v4/flower/video/from-image`, `flow → /v4/flow/video/from-ingredients` (v4, з `reference_images`), `grok → /v4/grok/video/from-image`. **V5 анімація:** `flow_omni_flash`, `flow_fast`, `flow_light` використовують `/api/v5/generations` з полем `inputs: [data_uri]` і операціями `_from_ingredients` (наприклад `flow_video_omni_flash_from_ingredients_10s`); `flow_quality` **не підтримує** image-to-video. Пріоритет анімації береться з `googler_video_priority` (окремого списку для анімації немає). `animate_image_with_priority` повертає `(provider_name, data_uri)` — провайдер потрібен для визначення `is_omni`. Провайдери, що не підтримують image-to-video, повертають помилку і пропускаються.
...
- **Конвертація в WAV та тривалість аудіо:** Якщо увімкнено `voiceover_convert_to_wav`, після `voice.mp3` запускається `ffmpeg -i voice.mp3 voice.wav`. MP3 при цьому **не видаляється** — залишаються обидва файли. Тривалість вимірюється вже з фінального файлу (WAV або MP3) через **ffprobe** (`get_audio_duration_secs` → `ffprobe -show_entries format=duration`). Важливо: не використовувати тут власний Rust-парсер замість ffprobe. Edge TTS розбиває довгий текст на чанки, а після склеювання через `ffmpeg -f concat -c copy` Xing VBR-заголовок **першого чанку** зберігається в результуючому MP3 — він містить кількість фреймів лише того чанку, а не всього файлу. Парсер що довіряє Xing-заголовку повертав би лише тривалість першого чанку (наприклад 13с замість 30с). ffprobe читає всі фрейми і завжди повертає правильне значення. Якщо FFmpeg недоступний — конвертація тихо пропускається, задача продовжується з MP3.
...
- **Діалог відновлення (`gui/pipeline/resume.rs`) — архітектура.**
  `FoundFiles::scan()` перевіряє конкретні файли: `text.txt`, `voice.mp3` / `voice.wav`, `subtitle.srt`, вміст `media/` (рахує зображення та відео окремо), `{safe_task_name}.mp4`. Якщо є `timeline.json` — додатково читається кількість сегментів (`segments.len()`) і зберігається у `expected_media: Option<usize>`. `safe_task_name` санітайзиться тією самою логікою що й у `montage.rs` (заміна спецсимволів на `_`).
...
- **Режим «Догенерувати відсутні» — окремий шлях в діалозі відновлення.** Якщо кількість файлів у `media/` менша за кількість сегментів у `timeline.json` (`expected_media`), у діалозі з'являється окрема широка кнопка **«Догенерувати відсутні (N)»** над стандартними кнопками. Клік викликає `enqueue_fill_missing`, яка: встановлює `resume_from_stage = Some(Video)`, `skip_agent_on_resume = true` (агент не перезапускається), `skip_existing_media = true` (існуючі файли пропускаються). У рядку медіа у діалозі показується рахунок `X зображень (5/12)` де `5` — є, `12` — очікується. Індикація рахунку доступна лише якщо `timeline.json` присутній і `expected_media` вдалось зчитати.
...
- **`video_agent_prompt` і `video_prompt` — два незалежних поля.** `video_prompt` — промт для Googler (генерація медіа), завжди відображається. `video_agent_prompt` — **додатковий** користувацький промт для CLI-агента, відображається лише при Claude Code, Gemini CLI, Codex CLI або AGY CLI. Зберігаються окремо в `AppSettings`, `PipelineTemplate`, `JobSettings`. Фінальний промт агента будується в `run_agent_timeline`: `VIDEO_AGENT_SYSTEM_PROMPT` (з `agent_prompts.rs`) + `"\n\n"` + `video_agent_prompt` (якщо не порожній). Плейсхолдери `{{srt}}` (шлях до `subtitle.srt`) та `{{path}}` (шлях до `timeline.json`) замінюються в обох частинах перед відправкою. Щоб змінити базову поведінку агента — редагуй лише `src/core/pipeline/agent_prompts.rs`.

- **Виклик Claude CLI (`claude`), Gemini CLI (`gemini`) та Codex CLI (`codex`)**:
  Індивідуальні моделі для CLI зберігаються та валідуються окремо. При виклику Gemini CLI критично важливим є використання прапорців `--output-format json` (для отримання чистої відповіді без ANSI та службових рядків), `--yolo` (неінтерактивність) та `--skip-trust` (довіра воркспейсу). На Windows та macOS CLI запускається **напряму** без проксування через `cmd /C` — бо вони є самостійними виконуваними файлами (`cmd /C` ламав передачу флагів). Для цього використовується функція `new_direct_cli_command` (`src/bundle.rs`), яка створює прямий `Command::new` на Windows та автоматично знаходить повний шлях до бінарника через `find_binary_macos` та додає розширений `PATH` на macOS, запобігаючи помилкам відсутності файлу.
  
  Для Codex CLI при запуску нової сесії та перекладу передаються аргументи `-` та інструкції через `stdin` (для запобігання обмеженням Windows командного рядка 8191 символів та пошкодження спецсимволів). Нова сесія запускається з флагом `--json` — Codex виводить NDJSON-події у stdout. `thread_id` береться з першої події `{"type":"thread.started","id":"..."}` замість ручного генерування UUID — це дозволяє точно ідентифікувати сесію для `exec resume`. У чат рендеруються: `[Bash] $cmd` (shell-виклики), `[->] output/filename` (результати команд та зміни файлів), `[STATS]` (токени/тривалість), стрімінг тексту агента. `call_codex_resume` також використовує `--json` + `parse_codex_json_response`. `call_codex` (переклад) — без `--json`, plaintext.
  ...
  - **Візуальне розділення режимів у секції «Відеоряд»:** Два незалежних перемикачі на радіо-кнопках: «Режим роботи» (API / Агент) та «Режим відеоряду» (Генерація / Стоки). Комбобокси сервісів фільтруються залежно від обраного режиму — користувач бачить лише релевантні опції. Під капотом `video_llm_service` та `video_service` залишаються звичайними рядками — режим обчислюється з них при рендері, збереження в шаблони не змінено.
  - **Приховування налаштувань нарізки тексту в агентному режимі:** При виборі «Агент» автоматично ховаються налаштування режиму нарізки тексту (агенти працюють з повним текстом). Режим нарізки для API/Агент **незалежний**: `text_split_mode` — активне значення, `text_split_mode_openrouter` — збережений вибір для API-режиму. При переході API→Агент зберігається, при Агент→API відновлюється.
...
- **macOS `.app` не успадковує PATH з терміналу — CLI-перевірки та запуски потребують явного PATH.** Скомпільований `.app`-бандл запускається з мінімальним PATH (`/usr/bin:/bin:/usr/sbin:/sbin`). `gemini` і `claude` встановлені через npm/brew у `/usr/local/bin/` або `/opt/homebrew/bin/` чи `~/.local/bin/` — тому `Command::new` просто провалюється. `welcome.rs` вирішує це хелперами: `find_binary_macos(name)` шукає бінарник у відомих місцях і повертає повний шлях; `macos_extended_path()` будує розширений PATH рядок для передачі у `.env("PATH", ...)`. Це критично для Node.js-скриптів (зокрема `gemini`, який є `#!/usr/bin/env node`): навіть якщо сам бінарник знайдено, `env` не знайде `node` без правильного PATH. Цей же механізм пошуку бінарників та розширення PATH інтегровано у запуск CLI-агентів через функцію `new_direct_cli_command` в `bundle.rs`. `cargo run` не має цієї проблеми бо запускається з терміналу де PATH вже розширений.
...
- **Асинхронне декодування зображень у Stock Picker**: Виклик `image::load_from_memory` є важкою синхронною операцією. Коли він виконувався в головному UI-потоці при відображенні результатів пошуку стоків, програма відчутно фрізила на 100-300мс при підвантаженні кожної групи мініатюр. Виправлено шляхом перенесення декодування у фонові робочі потоки, які одразу повертають повністю розкодований `egui::ColorImage`. Головний UI-потік у функції `flush_thumb_loading` займається виключно легким викликом `ctx.load_texture`.

- **Запобігання виходу вікон за межі екрана та авто-розширення у Stock Picker**: 
  - Вікна `stock_picker_single` та `stock_trim_editor` обмежені за допомогою `.constrain(true)` та `.max_size()`, що тримає їх всередині меж вікна програми.
  - Вертикальна `ScrollArea` у вікні стоків обмежена за висотою за допомогою `.max_height(screen.height() - 200.0)`, тому вона стискається зі скролбаром замість роздування вікна по вертикалі.
  - Кількість колонок `cols` для `egui::Grid` у Stock Picker тепер динамічно розраховується на основі `ui.available_width()` всередині вертикальної `ScrollArea` (без горизонтального скролу), що дозволяє вікну плавно стискатися до `min_size` (420px) без нескінченної петлі розширення (feedback loop).
  - Прев'ю-кадр у `stock_trim_editor` адаптивно обмежує висоту (`avail_h - 180.0`), запобігаючи вертикальному переповненню.
