...
- **Базова доріжка (`segments`):** `{start_secs, end_secs, media, zoom_enabled, shake_enabled}` з відносними шляхами. Враховуються тільки кліпи `track_idx=0` з призначеним файлом, відсортовані за `start_secs`. Порожні ділянки (gaps) — `"media": null`.
...
**Відтворення звуку (`AudioPlayer`, крейт `rodio`):**

`MontageEditorState` містить `pub(crate) audio_player: Option<AudioPlayer>`. `Some` = аудіо зараз грає; `None` = зупинено. Дроп `AudioPlayer` автоматично зупиняє відтворення.

- **Старт:** у блоці оновлення плейхеду `draw_montage_editor_window` (виконується кожен кадр поки `is_playing`): якщо `audio_player.is_none()` → `AudioPlayer::start(audio_path, playhead)`. Тобто аудіо запускається на першому кадрі після натискання ▶, а також після будь-якого скрабу.
- **`AudioPlayer::start(path, start_secs)`** — відкриває аудіофайл, створює `rodio::OutputStream` + `Sink`, декодує через `rodio::Decoder`, за потреби пропускає на `start_secs` вперед через `source.skip_duration()` (decoder перебирає семпли без рендерингу — швидко). Повертає `None` якщо пристрій недоступний або файл не читається — відеопрев'ю продовжує працювати без звуку.
- **Пауза/Стоп:** `audio_player = None` — дроп Sink/Stream зупиняє апаратний аудіо-потік. При паузі поточний `playhead` зберігається. При відновленні — аудіо відкривається знову з того ж `playhead` (seek через `skip_duration`).
- **Скраб (лінійка таймлінії):** якщо плейхед змістився більш ніж на 0.05с → `audio_player = None`. Якщо `is_playing` — на наступному кадрі плеєр стартує з нової позиції.
- **⏮ / ⏭:** також скидають `audio_player` для негайного ресинху.
- **Кінець файлу:** `playhead >= total_dur()` → `is_playing = false`, `audio_player = None`.
- **Тільки `voice.wav`/`voice.mp3`** (`editor.audio_path`). Аудіо-кліпи на таймлінії візуально відображаються, але не відтворюються в прев'ю.
- **`OutputStream` є `!Send`** на деяких платформах (Windows WASAPI/COM) — не є проблемою, бо `MontageEditorState` існує тільки на головному UI-потоці eframe.
