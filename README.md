...
│   │   ├── control.rs           — секція "Контроль" (перемикач контролю перекладу + чекбокс авто-відкриття вікна)
...
Реалізований у `src/core/pipeline/mod.rs` як крок 4 пайплайну після відеоряду.
...
- **Кнопка «▶ Запустити» блокується якщо модель Whisper не завантажена.** Перевіряється `whisper_model_exists()` для кожної `Pending`-задачі з увімкненими субтитрами. Якщо модель ще завантажується — показується `⏳` повідомлення через `on_disabled_hover_text`.
