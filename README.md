...
- **`enqueue_from_history(entry: &TaskHistoryEntry)`** — метод `impl VideoMakerApp`, викликається при кліку на картку в панелі историї. Будує `JobSettings` зі знімку налаштувань запису (`entry.settings`) не чіпаючи поточний стан панелі пайплайну. `voicebot_key` та `googler_key` беруться з поточного стану програми (їх нема в `PipelineTemplate`). Якщо в папці задачі вже є файли — відкриває стандартний діалог відновлення (`resume_dialog_open`, `resume_pending`); якщо папка чиста — одразу додає в чергу.
...
**Клік по картці** одразу додає задачу в чергу через `enqueue_from_history` — налаштування панелі пайплайну **не змінюються**. Задача запускається зі збереженим знімком налаштувань (`entry.settings`), текстом і назвою з историї. Якщо папка задачі вже містить проміжні файли — замість прямого додавання відкривається стандартний діалог відновлення (той самий, що і при «Додати в чергу» через кнопку). `draw_task_history_panel` повертає `Option<TaskHistoryEntry>` (раніше повертав `Option<(PipelineTemplate, String)>` і прокидав текст+налаштування в поля UI).
