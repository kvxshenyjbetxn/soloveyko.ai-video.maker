...
- **Виклик Claude CLI (`claude`) та Gemini CLI (`gemini`)**:
  Індивідуальні моделі для CLI зберігаються та валідуються окремо. При виклику Gemini CLI критично важливим є використання прапорців `--output-format json` (для отримання чистої відповіді без ANSI та службових рядків), `--yolo` (неінтерактивність) та `--skip-trust` (довіра воркспейсу). На Windows та macOS CLI запускається **напряму** через `Command::new("claude")` / `Command::new("gemini")` без проксування через `cmd /C` — бо `claude.exe` і `gemini` є самостійними виконуваними файлами. `cmd /C` ламав передачу флагів (зокрема `--permission-mode bypassPermissions`), через що агент запитував дозволи у тексті відповіді замість того щоб одразу записувати файли. Для агентних викликів Claude (нова сесія та --resume) додається `--permission-mode bypassPermissions` — без нього в `-p` (print) режимі Write-tool потребує інтерактивного підтвердження якого неможливо надати у subprocess.
  - **Робоча директорія CLI-агентів — кінцева папка задачі.** `call_claude_code` та `call_gemini_cli` приймають `working_dir: Option<&str>` і викликають `cmd.current_dir(dir)` якщо `Some`. З пайплайну та вікна контролю перекладу завжди передається `Some(&settings.save_path)` — тобто агенти відкриваються в папці `{save_path}/{task_name}/`. Це дозволяє агентам читати/записувати файли задачі відносними шляхами (`text.txt`, `media/` тощо).
  ...
  - **Автоматичне приховування налаштувань нарізки тексту:** Для Claude Code та Gemini CLI у секції відеоряду автоматично приховуються налаштування режиму нарізки, оскільки ці локальні інструменти за замовчуванням мають працювати виключно з повним текстом сценарію без нарізання на дрібні сегменти.
