...
- **Прев'ю + транспортні кнопки** (центр) — відображає кадр на поточній позиції плейхеду з ефектами зуму, покачування та переходами. **Активна доріжка визначається динамічно:** серед усіх візуальних кліпів (не аудіо) що покривають поточний плейхед, обирається той з найнижчим `track_idx` (найвищий пріоритет); якщо жодного — fallback на доріжку 0. Переходи між кліпами (prev/curr) розраховуються в межах тієї самої активної доріжки. Транспортні кнопки ⏮ ⏹ ▶/⏸ ⏭ розташовані **під відео**, над таймлінією. При відтворенні паралельно програється аудіо-трек (`voice.wav`/`voice.mp3`). Прев'ю центрується вручну через `pad_x = (avail_w - frame_w) / 2.0` (без `vertical_centered` — він спричиняв дублювання кнопок).
...
- **Overlay безпечної зони:** якщо shake увімкнено — пунктирна жовта обводка (`safe = rect.shrink2(Vec2(mx, my))`) показує мінімальну гарантовано видиму область при максимальному зміщенні. Без написів — щоб прев'ю відчувалось як фінальне відео.
...
**`save_to_timeline()`:**
Перед сигналом `montage_control_resume` — editor записує поточний стан таймлінії у `{save_path}/timeline.json`. Формат: `{start_secs, end_secs, media}` з відносними шляхами (forward slashes на обох ОС). Враховуються тільки кліпи доріжки 0 з призначеним файлом, відсортовані за `start_secs`. **Порожні ділянки між кліпами (gaps) зберігаються як null-сегменти** — `"media": null` з відповідними `start_secs`/`end_secs`. `run_montage` рендерить їх як чорний екран відповідної тривалості. `strip_prefix` використовує canonicalize-fallback для надійності на Windows (де шляхи можуть відрізнятись регістром або форматом). `run_montage` читає саме цей файл — тому всі зміни в редакторі гарантовано потрапляють у фінальне відео.
