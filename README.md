...
│   │   ├── preview.rs           — draw_preview: adaptive quality (scrub/sharp), UV-based зум/покачування, 11 груп xfade переходів, overlay compositing, transport controls, PreviewDragState для drag-трансформу overlay-кліпів
...
- **Прев'ю + транспортні кнопки** (центр) — відображає кадр на поточній позиції плейхеду з ефектами зуму, покачування, переходами та overlay-доріжками. Підтримує адаптивну якість: під час playback/скрабінгу показує легкий scrub-кадр, після зупинки плейхеду — автоматично підтягує чіткий still-кадр. **Базова доріжка (V1, `track_idx=0`)** рендерується першою з переходами. Поверх неї рендеруються всі overlay-кліпи (`track_idx≥1`) з їхніми `scale`/`pos_x`/`pos_y` трансформами. Транспортні кнопки ⏮ ⏹ ▶/⏸ ⏭ розташовані **під відео**, над таймлінією. При відтворенні паралельно програється аудіо-трек (`voice.wav`/`voice.mp3`). Прев'ю центрується вручну через `pad_x = (avail_w - frame_w) / 2.0` (без `vertical_centered` — він спричиняв дублювання кнопок). Клік/drag на прев'ю активує трансформ виділеного overlay-кліпу через `PreviewDragState`.
