...
| **Googler v5 (video)** | `POST /api/v5/generations` → `GET /api/v5/generations/{id}` → `GET storage.fast-gen.ai/{download_path}` | Провайдери: `flow_fast`, `flow_light`, `flow_quality`, `flow_omni_flash`; polling статусу до `"completed"` / `"failed"` |
