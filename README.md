...
- **Гранулярний прогрес генерації медіа у картках задач:** Під час виконання відеоряду мітка етапу показує `Відеоряд (3/10)` — скільки медіафайлів вже згенеровано зі загальної кількості. Прогресбар задачі відображає реальний відсоток (не умовні 50% "running"), оскільки `calculate_progress()` читає `media_progress`. Загальний прогресбар черги також автоматично стає точнішим.
