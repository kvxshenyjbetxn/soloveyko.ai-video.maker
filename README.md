...
│   │   ├── state.rs             — MontageEditorState: весь стан редактора; load() — завантаження timeline.json + пул + UUID-sync; save_to_timeline() — запис timeline.json з segments/overlay_tracks (зберігає оригінальні start_secs для overlap-зон); `overlap_transition_popup` — стан випадаючого списку вибору xfade-переходу
...
│   │   ├── preview.rs           — draw_preview: adaptive quality (scrub/sharp), UV-based зум/покачування, 11 груп xfade + overlap-кросфейд (автоматичний crossfade при накладанні кліпів), overlay compositing, transport controls, PreviewDragState для drag-трансформу overlay-кліпів
│   │   └── timeline.rs          — draw_timeline: ruler, N доріжок, sticky labels, drag Move/TrimLeft/TrimRight, магнітний снеп, ghost preview, кнопки «+ Відео»/«+ Аудіо», жовта сітка зон накладання (crosshatch) + кнопка ◈ вибору xfade-переходу (попап з 49 варіантами); приймає `regen_paths: &HashSet<PathBuf>`
