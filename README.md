...
**`save_to_timeline()`:**
Перед сигналом `montage_control_resume` — editor записує поточний стан таймлінії у `{save_path}/timeline.json`. Формат: `{start_secs, end_secs, media}` з відносними шляхами (forward slashes на обох ОС). Враховуються тільки кліпи доріжки 0 з призначеним файлом, відсортовані за `start_secs`. `run_montage` читає саме цей файл — тому всі зміни в редакторі гарантовано потрапляють у фінальне відео.

**«▶ Продовжити рендер»** — доступна лише коли задача в стані `AwaitingMontageControl`. При кліку: 1) `save_to_timeline()` записує зміни; 2) сигналізує `montage_control_resume` condvar — пайплайн продовжується до `run_montage`.
