...
- **`calculate_progress`** — метод структури `PipelineJob`, який динамічно розраховує поточний прогрес виконання завдання у діапазоні `[0.0..1.0]`, а також кількість завершених та загальних активних етапів. Етапи мають однакову вагу (наприклад, по 50% при увімкнених перекладі та озвучці). Статус `Done` дає `1.0` прогресу для етапу, а `Running` — `0.5` для плавного живого відображення ходу роботи. Метод спроектовано гнучким, що дозволяє легко додавати нові етапи пайплайну в майбутньому.
