...
- **Навігаційна панель (`navigation_bar`):** висота `min_height(40.0)` навмисно підібрана під рядок статистики редактора (розмір шрифту 16px). Вкладки і назва програми відображаються шрифтом 14px, вирівнювання по центру через `horizontal_centered`. Баланс-чіпи використовують пропорційний шрифт 13px (замість моноширинного Small) — це суттєво зменшує пробіли між елементами тексту. Відстань між чіпами: `item_spacing.x = 2.0`. Назва програми ("Soloveyko.AI-Video.Maker") з навбару прибрана — місце використовується для вкладок і балансів.
...
- **Googler-баланс у топбарі** відображає 8 значень в одному рядку (`img: X/Y vid: X/Y th: iX/Y vX/Y`). Префікси `i` (images) та `v` (video) перед потоками додані для наочності. Значення `/Y` — це `googler_image_max_threads` / `googler_video_max_threads` зі стану програми (локальний ліміт), а не серверне `threads_allowed`. Це зроблено свідомо, щоб чіп показував актуальні робочі ліміти а не серверні максимуми.
