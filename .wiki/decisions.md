...
| 2026-04-13 | MCP reverse tunnel запускається й завершується разом із desktop app | Потрібен стабільний доступ до локального MCP із VPS без ручного Tabby/терміналу та без накопичення фонових `ssh.exe` | Додано `MCPAutoForwardEnabled`, прихований запуск `startVPS.bat`, пошук живого tunnel по `ssh.exe` command line та kill на shutdown |
