...
import { GetWorkerStatus, ToggleWorkerMode, GetAvailableWorkers, GetMyHardwareID, PingWorker } from '../../../wailsjs/go/main/App';
...
export function RemoteControl() {
    ...
    const handlePing = async (hwId: string) => {
        if (pingingIds.has(hwId)) return;
        
        setPingingIds(prev => new Set(prev).add(hwId));
        showToast(t('other.ping_sent') || 'Запит на перевірку відправлено', 'info');
        
        try {
            await PingWorker(hwId);
            showToast(t('other.ping_success') || 'З\'єднання успішне', 'success');
        } catch (e) {
            console.error("Ping failed:", e);
            showToast(t('other.ping_error') || 'Помилка пінгу', 'error');
        } finally {
            setPingingIds(prev => {
                        ...
                        workers.map(worker => (
                            <div key={worker.id} className="worker-node">
                                ...
                                <div className="worker-actions">
                                    ...
                                    <button 
                                        className={`ping-btn ${pingingIds.has(worker.hardware_id) ? 'pinging' : ''}`}
                                        onClick={() => handlePing(worker.hardware_id)}
                                        title={t('other.ping_worker') || 'Перевірити зв\'язок'}
                                        disabled={pingingIds.has(worker.hardware_id)}
                                    >
